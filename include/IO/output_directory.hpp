...
struct IO_parameters // singleton
{
  ...
  public:
    ...
    void set_restart_directory(
        std::shared_ptr<dictionary_t>& _dict_output) noexcept
    {
        std::string              dir = "./";
        boost::mpi::communicator world;
    ...
    std::string restart_load_dir() const noexcept
    {
        return rel_directory_restartLoad_str;
    }
    std::string restart_save_dir() const noexcept
    {
        return rel_directory_restartSave_str;
    }

  private:
    boost::filesystem::path directory_output =
        boost::filesystem::current_path();
    std::string rel_directory_str = "./";

    boost::filesystem::path directory_restartLoad =
        boost::filesystem::current_path();
    std::string rel_directory_restartLoad_str = "./";

    boost::filesystem::path directory_restartSave =
        boost::filesystem::current_path();
    std::string rel_directory_restartSave_str = "./";
};

/** @brief Get a reference single instance  */
inline IO_parameters&
output()
{
...
{
    using dictionary_t = dictionary::Dictionary;
    IO_init(dictionary_t* dict_) { this->set(dict_); }
