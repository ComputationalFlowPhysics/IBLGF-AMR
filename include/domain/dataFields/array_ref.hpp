...
struct extent_generator
{
	...
	static extent_type<ND> apply(const Extent& index_base, 
                                 const Extent& extents)
	{
		return extent_generator<ND-1,NumDims>::apply(index_base, extents)
			[extent_range(index_base[ND-1], index_base[ND-1] + extents[ND-1])];
	}
};
...
{
	template<typename Extent>
	static extent_type<1> apply(const Extent& index_base, 
                                const Extent& extents)
	{
		return boost::extents[extent_range(index_base[0], 
                                           index_base[0] + extents[0])];
	}
};
...
{
	template<typename Extent>
	static index_gen_type<ND,ND> apply(const Extent& index_base, 
                                       const Extent& extents, 
                                       const Extent& strides)
	{
		return index_generator<ND-1,NumDims>::apply(index_base,extents,strides)
			[index_range(index_base[ND-1], 
	...
	static index_gen_type<ND,ND> apply(const Extent& index_base, 
                                       const Extent& extents)
	{
		return index_generator<ND-1,NumDims>::apply(index_base, extents)
			[index_range(index_base[ND-1], index_base[ND-1] + extents[ND-1])];
	}
};
...
{
	template<typename Extent>
	static index_gen_type<1,1> apply(const Extent& index_base, 
                                     const Extent& extents, 
                                     const Extent& strides)
	{
		return boost::indices[index_range(index_base[0], 
                              index_base[0] + extents[0], strides[0])];
	}
...
public: //member types
	...
	using view_type = typename boost_multi_array_ref::template 
                                                    array_view<Dim>::type;
...
public: //Ctors:
    ...
    :multi_array_ref_(_d, 
            detail::extent_generator<Dim,Dim>::apply(_domain.base(), 
                _domain.extent()), 
    ...
    {
        return  multi_array_ref_[
            detail::index_generator<Dim,Dim>::apply(_b.base(), 
                                                    _b.extent(), 
                                                    _stride)];
    }
    ...
    {
        return  multi_array_ref_[ 
            detail::index_generator<Dim,Dim>::apply(_b.base(), _b.extent())];
    }
