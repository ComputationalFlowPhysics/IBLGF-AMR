...
def run(config_path: str | Path, cli_overrides: Sequence[str] | None = None):
    bindings = load_bindings("iblgf_bindings_ns_amr")
    return bindings.ns_amr.run(str(Path(config_path)), list(cli_overrides or []))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the 3D NS AMR LGF pybind interface."
    )
    parser.add_argument("config_path", help="Path to the NS AMR config file.")
    parser.add_argument(
        "--simulation-override",
        action="append",
        default=[],
...
def main() -> int:
    ...
    print(result.fine_u3_linf_error)
    return 0
