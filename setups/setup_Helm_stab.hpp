...
#ifndef IBLGF_INCLUDED_SETUP_HELM_STAB_HPP
#define IBLGF_INCLUDED_SETUP_HELM_STAB_HPP
...
#include <iblgf/solver/Newton/Helm_stab.hpp>
...
class SetupHelmStab
: private crtp::Crtps<Setup, SetupHelmStab<Setup, SetupTraits>>
, public SetupTraits
{
  ...
  public: //default fields
    ...
    (
      ...
      (g_i,                 float_type,  2,  1,  1,  face,true),
      (r_i,                 float_type,  2,  1,  1,  face,true),
      (cell_aux,            float_type,  1,  1,  1,  cell,true),
      (cell_aux2,           float_type,  1,  1,  1,  cell,true),
      (face_aux,            float_type,  2,  1,  1,  face,true),
      (face_aux2,           float_type,  2,  1,  1,  face,true),
      (stream_f,            float_type,  3,  1,  1,  edge,true),
      (edge_aux,            float_type,  3,  1,  1,  edge,true),
      //Fields for Jacobian
      (u_i_bc,              float_type,  2,  1,  1,  face,true),
      (nonlinear_tmp,       float_type,  2,  1,  1,  face,true),
      (face_aux_tmp,        float_type,  2,  1,  1,  face,true),
      (laplacian_face,      float_type,  2,  1,  1,  face,true),
      //(R_1,                 float_type,  Dim,  1,  1,  face,true),
      (nonlinear_tmp1,      float_type,  2,  1,  1,  face,true),
      (idx_u,               float_type,  2,  1,   1, face,true),
      (idx_p,               float_type,  1,  1,   1, cell,true),
      (idx_w,               float_type,  3,  1,   1, edge,true),
      (idx_N,               float_type,  2,  1,   1, face,true), //local index for nonlinear term and forcing smeared entries
      ...
      (idx_uz,              float_type,  1,  1,   1, cell,true), //z velocity
      (idx_Nz,              float_type,  1,  1,   1, cell,true), //z component nonlinear term
      (idx_u_g,             float_type,  2,  1,   1, face,true),
      (idx_p_g,             float_type,  1,  1,   1, cell,true),  
      (idx_w_g,             float_type,  3,  1,   1, edge,true),
      (idx_N_g,             float_type,  2,  1,   1, face,true),
      (idx_cs_g,            float_type,  1,  1,   1, cell,true),  //global index across all processors
      (idx_uz_g,            float_type,  1,  1,   1, cell,true),
      ...
      (conj_r_cell_aux,         float_type,  1,  1,  1,  cell,true),
      ...
      (r_i_tmp,             float_type,  2,  1,  1,  face,true),
  ...
  public: //Trait types to be used by others
    ...
    using Fmm_t = Fmm<SetupHelmStab>;
    using fmm_mask_builder_t = FmmMaskBuilder<domain_t>;
    using poisson_solver_t = solver::PoissonSolver<SetupHelmStab>;
  ...
  public: //Ctors
    ...
    SetupHelmStab(Dictionary* _d, domaint_init_f _fct,
