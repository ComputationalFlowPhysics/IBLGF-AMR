...
class Client : public ClientBase<ServerClientTraits<Domain>>
{
  public:
    ...
    using induced_fields_task_t =
        typename trait_t::template induced_fields_task_t<BufferPolicy>;
    using acc_induced_fields_task_t =
        typename trait_t::acc_induced_fields_task_t;
    ...
    using halo_communicators_tuple_t =
        typename tuple_utils::make_from_tuple<halo_communicator_template_t,
            fields_tuple_t>::type;
  ...
  public:
    ...
    void update_field(ClientUpdate& _update)
    {
             ...
             ++field_idx)
        {
            ...
            for (auto& key : _update.send_octs)
            {
                ...
                auto send_ptr =
                    it->data()->template get<Field>(field_idx).data_ptr();
    ...
    {
        auto& send_comm = task_manager_->template send_communicator<
            induced_fields_task_t<InfluenceFieldBuffer>>();
        auto& recv_comm = task_manager_->template recv_communicator<
            induced_fields_task_t<InfluenceFieldBuffer>>();
        ...
        bool _start_communication, int fmm_mask_idx)
    {
        ...
        if (!it->locally_owned())
        {
            ...
            {
                auto send_ptr =
                    it->data()->template get<SendField>().data_ptr();
        ...
        else
        {
            ...
            {
                const auto recv_ptr =
                    it->data()->template get<RecvField>().data_ptr();
    ...
    template<class SendField, class RecvField>
    void check_combined_induced_field_communication(
        bool _finish = false) noexcept
    {
        auto& send_comm = task_manager_->template send_communicator<
            induced_fields_task_t<InfluenceFieldBuffer>>();
        auto& recv_comm = task_manager_->template recv_communicator<
        ...
        while (true)
        {
            send_comm.unpack_messages();
            recv_comm.unpack_messages();
            if (!_finish || (recv_comm.done() && send_comm.done())) break;
        }
    ...
    void initialize_halo_communicators() noexcept
    {
        ...
        {
            tuple_utils::for_each(halo_communicators_[l],
                [&](auto& hcomm) { hcomm.compute_tasks(domain_, l); });
        }
