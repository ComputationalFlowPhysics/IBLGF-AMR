...
class fourier_cont_intrp
{
  public: // constructor
    ...
    , nli_aux_1d_antrp_tmp(std::array<size_t, 1>{{Nb_}})
    {
        ...
        antrp_mat_sub_[0].data_ =
            xt::view(antrp_mat_.data_, xt::range(0, Nb_), xt::range(0, Nb_));
        antrp_mat_sub_[1].data_ = xt::view(
            antrp_mat_.data_, xt::range(0, Nb_), xt::range(Nb_, 2 * Nb_));
  ...
  public: // functionalities
    ...
    void add_source_correction(auto parent, double dx)
    {
        ...
        for (int i = 0; i < parent->num_children(); ++i)
        {
            ...
            xt::xtensor<float_type, 3> child_target_L_tmp(
                std::array<size_t, 3>{{Nb_, Nb_, Nb_}});
            ...
            for (int i = 1; i < Nb_ - 1; ++i)
            {
                for (int j = 1; j < Nb_ - 1; ++j)
                {
                    for (int k = 1; k < Nb_ - 1; ++k)
                    {
                        ...
                        child_target_L_tmp(k, j, i) =
                            -6.0 * child_target_tmp(k, j, i);
                        child_target_L_tmp(k, j, i) +=
                            child_target_tmp(k, j, i - 1);
                        child_target_L_tmp(k, j, i) +=
                            child_target_tmp(k, j, i + 1);
                        child_target_L_tmp(k, j, i) +=
                            child_target_tmp(k, j - 1, i);
                        child_target_L_tmp(k, j, i) +=
                            child_target_tmp(k, j + 1, i);
                        child_target_L_tmp(k, j, i) +=
                            child_target_tmp(k + 1, j, i);
                        child_target_L_tmp(k, j, i) +=
                            child_target_tmp(k - 1, j, i);
                    }
                }
    ...
    void nli_intrp_node(auto& child, auto& parent, int child_idx)
    {
        ...
        for (int q = 0; q < n; ++q)
        {
            for (int l = 0; l < n; ++l)
            {
                ...
                xt::noalias(view(nli_aux_2d_intrp, l, xt::all())) =
                    xt::linalg::dot(
                        nli_aux_1d_intrp, antrp_mat_sub_[idx_x].data_);
            }
            ...
            for (int l = 0; l < n; ++l)
            {
                ...
                xt::noalias(view(nli_aux_3d_intrp, q, xt::all(), l)) =
                    xt::linalg::dot(
                        nli_aux_1d_intrp, antrp_mat_sub_[idx_y].data_);
            }
        }
        ...
        for (int p = 0; p < n; ++p)
        {
            for (int q = 0; q < n; ++q)
            {
                ...
                xt::noalias(view(child, xt::all(), p, q)) += xt::linalg::dot(
                    nli_aux_1d_intrp, antrp_mat_sub_[idx_z].data_);
            }
        }
    }
        ...
        for (int q = 0; q < n; ++q)
        {
            for (int l = 0; l < n; ++l)
            {
                ...
                xt::noalias(view(nli_aux_2d_antrp, xt::all(), l)) =
                    xt::linalg::dot(
                        antrp_mat_sub_[idx_z].data_, nli_aux_1d_antrp_tmp);
                //xt::noalias( view(nli_aux_2d_antrp, xt::all(), l) ) =
        ...
        for (int p = 0; p < n; ++p)
        {
            for (int q = 0; q < n; ++q)
            {
                ...
                nli_aux_1d_antrp_tmp =
                    view(nli_aux_3d_antrp, xt::all(), p, q) * 1.0;

                xt::noalias(view(parent, p, q, xt::all())) += xt::linalg::dot(
                    antrp_mat_sub_[idx_x].data_, nli_aux_1d_antrp_tmp);
  ...
  private:
    void antrp_mat_calc(auto& antrp_mat_, int Nb_)
    {
        ...
        double M(Nb_);
        //M=16.0;
        xt::xtensor<std::complex<double>, 2> mat_cal_basis(
            std::array<size_t, 2>{{Nb_, int(M)}});
        xt::xtensor<std::complex<double>, 2> mat_cal_intrp(
            std::array<size_t, 2>{{2 * Nb_ - 1, int(M)}});
        ...
        auto tmp =
            xt::linalg::dot(mat_cal_intrp, xt::linalg::inv(mat_cal_basis));
  ...
  private:
    xt::xtensor<float_type, 1>
        nli_aux_1d_intrp; //(std::array<size_t, 1>{{ n }});
    xt::xtensor<float_type, 2>
        nli_aux_2d_intrp; //(std::array<size_t, 2>{{ n,n }});
    xt::xtensor<float_type, 3>
        nli_aux_3d_intrp; //(std::array<size_t, 3>{{ n,n,n }});

    xt::xtensor<float_type, 2>
        nli_aux_2d_antrp; //(std::array<size_t, 2>{{ n,n }});
    xt::xtensor<float_type, 3>
        nli_aux_3d_antrp; //(std::array<size_t, 3>{{ n,n,n }});
    xt::xtensor<float_type, 1>
        nli_aux_1d_antrp_tmp; //(std::array<size_t, 3>{{ n,n,n }});
};
