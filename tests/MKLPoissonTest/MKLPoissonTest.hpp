	...
	float_type run()
	{
        ...
        if (print_mat)
        {
            for (int k = 1; k < world.size(); k++)
            {
                if (world.rank() == k)
                {
                    for (int i = 1; i < ifherk.mat.numRow_loc(); i++)
                    {
                        std::cout << i << " ";
                        ifherk.mat.print_row(i);
                    }
                }
                world.barrier();
            }
        ...
        /*for (int i = 0; i < world.size();i++) {
            if (world.rank() == i) std::cout << "Rank " << i << " total dim is " << ndim << std::endl;
        }*/
        ...
        if (world.rank() != 0) {
            ...
            ifherk.mat.getCSR(ia, ja, a);
            ifherk.Grid2CSR<u_type, p_type>(b);
        }
        else {
            iparm[40] = 2;
			iparm[41] = 1;
       ...
       so MPI processes with rank 0 and 1 keep only part of solution */
        ...
        ifherk.CSR2Grid<u_num_type, p_num_type>(x);
