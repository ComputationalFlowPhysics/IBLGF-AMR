...
class NewtonIteration
{
  public: //member types
    ...
    , lsolver(_simulation)
    {
        ...
        std::fill(forcing_old.begin(), forcing_tmp.end(), tmp_coord);
...
private:
    ...
    void Solve_Jacobian() noexecpt {
        //TODO: solver to solve the inverse of Jacobian with the vectors
    }


    void Jacobian(float_type _alpha) noexcept
    {
        auto client=domain_->decomposition().client();

        clean<r_i_type>(); //use r_i as the result of applying Jcobian in the first block 
        clean<cell_aux_type>(); //use cell aux_type to be the second block
        forcing_tmp.resize(0); //use forcing tmp to store the last block, 
                               //use forcing_old to store the forcing at previous Newton iteration

        curl<u_i_type, edge_aux_type>();

        domain_->client_communicator().barrier();
        mDuration_type t_lgf(0);
        TIME_CODE( t_lgf, SINGLE_ARG(
                    Vel_from_vort<edge_aux_type, u_i_bc_type>;
                    ));
        //domain_->client_communicator().barrier();
        pcout<< "BCs solved in "<<t_lgf.count() << std::endl;

        copy_base_level_BC<u_i_bc_type, u_i_type>();

        domain_->client_communicator().barrier();

        laplacian<u_i_type, laplacian_face_type>();

        nonlinear_jac<u_type, u_i_type, g_i_type>();

        add<g_i_type, r_i_type>(-1);
        add<laplacian_face_type, r_i_type>(1.0/Re);

        clean<face_aux_tmp_type>();
        gradient<d_i_type, face_aux_tmp_type>();
        add<face_aux_tmp_type, r_i_type>();

        lsolver.template smearing<face_aux_tmp_type>(forcing_old, false);

        add<face_aux_tmp_type, r_i_type>(1.0);

        divergence<u_i_type, cell_aux_type>();

        lsolver.template projection<u_i_type>(forcing_tmp); //need to change this vector in the bracket
    ...
    void Vel_from_vort()
    {
        ...
        //clean<Velocity_out>();
        clean_leaf_correction_boundary<Source>(domain_->tree()->base_level(), true, 2);
                ...
                l < l_max; ++l)
        {
                    ...
                    it != domain_->end(l); ++it)
            {
                ...
                const auto dx_level =  dx_base/math::pow2(it->refinement_level());
                    domain::Operator::curl_transpose<stream_f_type,Target>( it->data(),dx_level, -1.0);
            }
        }
    ...
    void nonlinear_jac(float_type _scale = 1.0) noexcept
    {
             ...
             l < domain_->tree()->depth(); ++l)
        {
            ...
            for (auto it = domain_->begin(l); it != domain_->end(l); ++it)
            {
                ...
                const auto dx_level =
                    dx_base / math::pow2(it->refinement_level());
                domain::Operator::curl<Source_new, edge_aux_type>(
                    it->data(), dx_level);
            }
        }
             ...
             l < domain_->tree()->depth(); ++l)
        {
            ...
            for (auto it = domain_->begin(l); it != domain_->end(l); ++it)
            {
                ...
                domain::Operator::nonlinear<face_aux_tmp_type, edge_aux_type, nonlinear_tmp_type>(
                    it->data());
             ...
             l < domain_->tree()->depth(); ++l)
        {
            ...
            for (auto it = domain_->begin(l); it != domain_->end(l); ++it)
            {
                ...
                const auto dx_level =
                    dx_base / math::pow2(it->refinement_level());
                domain::Operator::curl<Source_old, edge_aux_type>(
                    it->data(), dx_level);
            }
        }
             ...
             l < domain_->tree()->depth(); ++l)
        {
            ...
            for (auto it = domain_->begin(l); it != domain_->end(l); ++it)
            {
                ...
                domain::Operator::nonlinear<face_aux_tmp_type, edge_aux_type, Target>(
                    it->data());
    ...
    void laplacian() noexcept
    {
             ...
             l < domain_->tree()->depth(); ++l)
        {
            ...
            for (auto it = domain_->begin(l); it != domain_->end(l); ++it)
            {
                ...
                const auto dx_level =
                    dx_base / math::pow2(it->refinement_level());
                domain::Operator::laplace<Source, Target>(
                    it->data(), dx_level);
            }
    ...
    float_type dotProduct() noexcept
    {
        ...
        {
            if (!it->locally_owned() || !it->has_data()) continue;
            float_type m _tmp = domain::Operator::blockDot<Field1, Field2>(it->data());
            m += m_tmp;
        }
