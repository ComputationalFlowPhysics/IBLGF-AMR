...
struct parameters
{
    ...
    static constexpr std::size_t N_tv = 1;
    ...
    Dim,
     (
        ...
        //IF-HERK
         ...
         (f_hat_re,       float_type,  Dim*Nf,  1,  1,  face,false), //need so ifherk_linear compiles
		 (f_hat_im,       float_type,  Dim*Nf,  1,  1,  face,false),
		 (u_hat_re            , float_type, 2*Nf,    1,       1,     face,false  ),
		 (u_hat_im            , float_type, 2*Nf,    1,       1,     face,false  ),
         (vec_u_re            , float_type, 2*N_tv,    1,       1,     face,true  ),
         (vec_u_im            , float_type, 2*N_tv,    1,       1,     face,true  ),
         (vec_check_re            , float_type, 2*N_tv,    1,       1,     face,true  ),
         (vec_check_im            , float_type, 2*N_tv,    1,       1,     face,true  ),
...
struct NS_AMR_LGF : public SetupLinear<NS_AMR_LGF, parameters>
{
          ...
          { return this->initialize_domain(_d, _domain); })
    {
        ...
        U_[0] =
            simulation_.dictionary()->template get_or<float_type>("Ux", 1.0);
        U_[1] =
            simulation_.dictionary()->template get_or<float_type>("Uy", 0.0);
        if (domain_->dimension() > 2)
            U_[2] = simulation_.dictionary()->template get_or<float_type>("Uz",
                0.0);

        smooth_start_ = simulation_.dictionary()->template get_or<bool>(
            "smooth_start", false);

        vortexType =
            simulation_.dictionary()->template get_or<int>("Vort_type", 0);
            ...
            [this](std::size_t idx, float_type t, auto coord = {0, 0})
        {
            ...
            float_type r =
                std::sqrt((coord[0] * coord[0] + coord[1] * coord[1]));
            ...
            else
            {
                ...
                else if (idx == 1) { f_alpha = -coord[0] / r / r * Omega; }
                else { f_alpha = 0.0; }
            }

            if (t <= 0.0 && smooth_start_) return 0.0;
        ...
        v_delta_ = simulation_.dictionary()->template get_or<float_type>(
            "vDelta", 0.2 * R_);
        single_ring_ = simulation_.dictionary()->template get_or<bool>(
            "single_ring", true);
        perturbation_ = simulation_.dictionary()->template get_or<bool>(
            "perturbation", false);
        vort_sep = simulation_.dictionary()->template get_or<float_type>(
            "vortex_separation", 1.0 * R_);
        hard_max_refinement_ = simulation_.dictionary()->template get_or<bool>(
            "hard_max_refinement", false);
        non_base_level_update = simulation_.dictionary()->template get_or<bool>(
            "no_base_level_update", false);
        NoMeshUpdate = simulation_.dictionary()->template get_or<bool>(
            "no_mesh_update", false);
        ...
        ctr_dis_x =
            0.0 *
            dx_; //this is setup as the center of the vortex in the unit of grid spacing
        ctr_dis_y = 0.0 * dx_;

        bool use_fat_ring =
            simulation_.dictionary()->template get_or<bool>("fat_ring", false);
        if (use_fat_ring)
            vr_fct_ = [this](float_type x, float_type y, int field_idx,
                          bool perturbation) {
                return this->vortex_ring_vor_fat_ic(x, y, field_idx,
                    perturbation);
            };
        else
            vr_fct_ = [this](float_type x, float_type y, int field_idx,
                          bool perturbation)
            { return this->vortex_ring_vor_ic(x, y, field_idx, perturbation); };
        ...
        base_threshold_ = simulation_.dictionary()->template get_or<float_type>(
            "base_level_threshold", 1e-4);

        nLevelRefinement_ =
            simulation_.dictionary_->template get_or<int>("nLevels", 0);

        hard_max_level_ = simulation_.dictionary()->template get_or<int>(
            "hard_max_level", nLevelRefinement_);

        global_refinement_ = simulation_.dictionary_->template get_or<int>(
            "global_refinement", 0);

        n_march_per_iter_ = simulation_.dictionary()->template get_or<int>(
            "n_march_per_iter", 10);
        ...
        domain_->register_adapt_condition() =
            [this](std::vector<float_type> source_max, auto& octs,
                std::vector<int>& level_change) {
                return this->template adapt_level_change(source_max, octs,
                    level_change);
            };
        ...
        domain_->ib().init(_d->get_dictionary("simulation_parameters"),
            domain_->dx_base(), nLevelRefinement_ + nIB_add_level_, Re_);

        if (!use_restart()&&!use_tree_)
        {
            domain_->init_refine(nLevelRefinement_, global_refinement_,
                nIB_add_level_);
        }
        else
        {
    ...
    float_type run(int argc, char* argv[])
    {
        ...
        // ifherk(&this->simulation_);
        ifherk_ = std::make_shared<time_integration_t>(&this->simulation_);
        ifherk_->Assign_idx<idx_u_type, idx_u_g_type>(); //assigns indices
        GetIdx();                                        //retrieves max index on each processor
        ifherk_->init_single_step();
        ifherk_->time_step_once();
        run_stab();
    ...
    float_type run_test(int argc, char* argv[])
    {
        boost::mpi::communicator world;
        PetscCall(SlepcInitialize(&argc, &argv, (char*)0, NULL));
        ...
        ifherk_->init_single_step();
        GetIdx();                                        //retrieves max index on each processor
        //testing random field to vector to field
        perturbIC<vec_u_re_type>();
        perturbIC<vec_u_im_type>();
        ifherk_->normalize_field_complex<vec_u_re_type, vec_u_im_type>();
        world.barrier();
        test_grid2vec2grid();
        world.barrier();
        ifherk_->clean<u_check_type>();
        ifherk_->add<vec_u_re_type, u_check_type>();
        ifherk_->add<vec_f_re_type, u_check_type>(-1.0);
        simulation_.write("final"); //u_check_type has error
        //testing random vector to field
        test_vec2grid();
        world.barrier();
        simulation_.write("final2");
        // ifherk_->clean<u_type>();
        // ifherk_->add<vec_f_re_type, u_type>();
        // ifherk_->time_step_once();
        // simulation_.write("final2");
        PetscCall(PetscFinalize());
    }

    float_type test_vec2grid()
    {
        boost::mpi::communicator world;
        ...
        PetscCall(PetscCommDuplicate(PETSC_COMM_WORLD, &petsc_comm, NULL));
        ...
        boost::mpi::all_reduce(world, max_local_idx, tot_global, std::plus<int>());
        tot_global += 1;                                 //add one since max_local_idx of 0 is -1
        int m_local = max_local_idx - min_local_idx + 1; //rank 0 has m_local = 1

        int m_max;
        boost::mpi::all_reduce(world, m_local, m_max, boost::mpi::maximum<int>());
        world.barrier();
        ...
        M = m_max * (size - 1);
        Vec x, y;
        PetscCall(VecCreate(petsc_comm, &x));
        ...
        //print out ownership range on each rank
        std::cout << "rank " << rank << " rstartx " << rstartx << " rendx " << rendx << std::endl;
        PetscCall(VecDuplicate(x, &y));
        vec2grid<idx_u_type, vec_check_re_type, vec_check_im_type>(x, 0); //vec_check is x

        Mat A;
        PetscCall(MatCreateShell(petsc_comm, m, m, PETSC_DECIDE, PETSC_DECIDE, NULL, &A));
        NS_AMR_LGF* ctx = this;
        PetscCall(MatShellSetContext(A, ctx));
        PetscCall(MatShellSetOperation(A, MATOP_MULT, (void (*)(void))MatMult_matrix));

        PetscCall(MatMult(A, x, y));
        ifherk_->clean<vec_f_re_type>();
        ifherk_->clean<vec_f_im_type>();
        vec2grid<idx_u_type, vec_f_re_type, vec_f_im_type>(y, 0); //vec_f is y=x+dt

        getRankfromVec<idx_u_type,u_type>(y, 0);
    ...
    float_type test_grid2vec2grid()
    {
        ...
        grid2vec<idx_u_type, vec_u_re_type, vec_u_im_type>(x, 0);
        vec2grid<idx_u_type, vec_f_re_type, vec_f_im_type>(x, 0);
        getRankfromVec<idx_u_type,u_type>(x, 0);
        PetscCall(VecDestroy(&x));
        PetscCall(VecDestroy(&y));
    ...
    float_type run_stab()
    {
        ...
        EPS eps;
        PetscCall(EPSCreate(petsc_comm, &eps));
        PetscCall(EPSSetOperators(eps, A, NULL));
        PetscCall(EPSSetProblemType(eps, EPS_NHEP));
        PetscCall(EPSSetWhichEigenpairs(eps, EPS_LARGEST_MAGNITUDE));

        // PetscScalar sigma = 0.99+0.1*PETSC_i;
        // PetscCall(EPSSetTarget(eps,sigma));  // Set shift (change 0.0 to your desired shift)
        // PetscCall(EPSMonitorSet(eps, EPSMonitorFirst));
        // PetscCall(EPSMonitor(eps,EPSMONITORFIRST,NULL,NULL));
        // PetscCall(EPSSetTolerances(eps, 1e-6, 2000));
        PetscCall(EPSSetTolerances(eps, 1e-2, 20));

        PetscCall(EPSSetDimensions(eps, 20, PETSC_DECIDE, PETSC_DECIDE));

        PetscViewer           viewer = NULL;
        PetscViewerAndFormat* viewer_format = NULL;
        if (rank == 0)
        {
            PetscCall(PetscViewerASCIIOpen(PETSC_COMM_SELF, "eig_conv.txt", &viewer));
            PetscCall(PetscViewerAndFormatCreate(viewer, PETSC_VIEWER_ASCII_DENSE, &viewer_format));
            PetscCall(EPSMonitorSet(eps,
                (PetscErrorCode(*)(EPS, PetscInt, PetscInt, PetscScalar*, PetscScalar*, PetscReal*, PetscInt,
                    void*))EPSMonitorAll,
                (void*)viewer_format, NULL));
        }
        // EPSSetType(eps,EPS_GMRES);
        ...
        world.barrier();
        //save first 2 eigenvectors
        Vec xr;
        PetscCall(VecDuplicate(x, &xr));
        //add real and imaginary components

        for (std::size_t i = 0; i < nconv; i++)
        {
            PetscCall(EPSGetEigenpair(eps, i, &kr, NULL, xr, NULL));
            if (rank == 0) { std::cout << "Eigenvalue " << i << ": " << kr << std::endl; }
            vec2grid<idx_u_type, vec_u_re_type, vec_u_im_type>(xr, 0);
            world.barrier();
            simulation_.write("eigenvector_" + std::to_string(i));
        }
    ...
    static PetscErrorCode MatMult_matrix(Mat A, Vec x, Vec y)
    {
        NS_AMR_LGF* ctx;
        PetscCall(MatShellGetContext(A, &ctx));
        ctx->MatMult_matrix_free(A, x, y);
    }

    PetscErrorCode MatMult_matrix_free(Mat A, Vec x, Vec y)
    {
        boost::mpi::communicator world;

        vec2grid<idx_u_type, vec_u_re_type, vec_u_im_type>(x, 0);
        // x and y are complex, A is real
        // x -> vec_u
        // time step re(vec_u)
        for (int dd = 0; dd < domain_->dimension(); dd++) { copy_idx<vec_u_re_type, u_type>(dd * N_tv, dd); }
        //time step u
        ifherk_->time_step_once(n_march_per_iter_,true);
        world.barrier();
        ifherk_->clean<vec_u_re_type>();
        for (int dd = 0; dd < domain_->dimension(); dd++) { copy_idx<u_type, vec_u_re_type>(dd, dd * N_tv); }

        ifherk_->clean<u_type>();
        for (int dd = 0; dd < domain_->dimension(); dd++) { copy_idx<vec_u_im_type, u_type>(dd * N_tv, dd); }
        ifherk_->time_step_once(n_march_per_iter_,true);
        world.barrier();
        //time step u
        ifherk_->clean<vec_u_im_type>();
        for (int dd = 0; dd < domain_->dimension(); dd++) { copy_idx<u_type, vec_u_im_type>(dd, dd * N_tv); }
        ifherk_->clean<u_type>();
        world.barrier();
        //time step u
        //time step u
        // time step im(vec_u)
        // vec_u -> u
        grid2vec<idx_u_type, vec_u_re_type, vec_u_im_type>(y, 0);
        world.barrier();
    }
    ...
    template<class Field_idx, class Field_tv>
    int getRankfromVec(Vec x,int idx)
    {
        boost::mpi::communicator world;
        //loop through points
        ...
        for (int l = base_level; l < domain_->tree()->depth(); l++)
        {
            ...
            const double w_1_2 =1.0;//std::pow(dx_level, domain_->dimension()/2.0);
                ...
                if (it->is_leaf() && !it->is_correction())
                {
                    for (std::size_t field_idx = 0; field_idx < Dim; ++field_idx)
                    {
                        for (auto& n : it->data())
                        {
                            ...
                            int rank=world.rank();
                            n(Field_tv::tag(), field_idx * N_tv + idx) = rank; //PetscRealPart(value)/w_1_2;
                        }
                    }
                }
            }
    ...
    int vec2grid(Vec x, int idx)
    {
        ...
        {
            const auto dx_level = domain_->dx_base() / math::pow2(l);
            const auto w_1_2 =1.0;//std::pow(dx_level, domain_->dimension()/2.0);
            for (auto it = domain_->begin(l); it != domain_->end(l); ++it)
            {
                if (!it->locally_owned() || !it->has_data()) continue;
                // if (!it->data().is_allocated()) continue;
    ...
    template<class Field_idx, class Field_re, class Field_im>
    int vec2grid(Vec xr, Vec xi,int idx)
    {
        //loop through points
        const PetscComplex* xr_array;
        PetscCall(VecGetArrayRead(xr, &xr_array));
        const PetscComplex* xi_array;
        PetscCall(VecGetArrayRead(xi, &xi_array));
        int base_level = domain_->tree()->base_level();
        for (int l = base_level; l < domain_->tree()->depth(); l++)
        {
            const auto dx_level = domain_->dx_base() / math::pow2(l);
                ...
                if (it->is_leaf() && !it->is_correction())
                {
                    for (std::size_t field_idx = 0; field_idx < Dim; ++field_idx)
                    {
                        for (auto& n : it->data())
                        {
                            ...
                            // int i_global =
                            //     (rank - 1) * m_local + i_local - 1;
                            PetscScalar valuer = xr_array[i_local];
                            PetscScalar valuei = xi_array[i_local];
                            // PetscCall(MatGetValuesLocal(B, 1, &i_local, 1, &j, &value));
                            // value = x_array[i_local];

                            n(Field_re::tag(), field_idx * N_tv + idx) = PetscRealPart(valuer)/w_1_2;
                            n(Field_im::tag(), field_idx * N_tv + idx) = PetscRealPart(valuei)/w_1_2;
                        }
                    }
                }
            }
        ...
        VecRestoreArrayRead(xi, &xi_array);
        return 0;
    }
    template<class Field_idx, class Field_re, class Field_im>
    int grid2vec(Vec x, int idx)
    {
        if (domain_->is_server()) return 0;
        //loop through points
        PetscInt size;
                ...
                if (it->is_leaf() && !it->is_correction())
                {
                    for (std::size_t field_idx = 0; field_idx < Dim; ++field_idx)
                    {
                        for (auto& n : it->data())
                        {
                            ...
                            //     (rank - 1) * m_local + i_local - 1;
                            // PetscComplex value;
                            PetscScalar value_r = n(Field_re::tag(), field_idx * N_tv + idx)*w_1_2;
                            PetscScalar value_i = n(Field_im::tag(), field_idx * N_tv + idx)*w_1_2;
                            PetscComplex value = value_r + PETSC_i * value_i;
                            if (i_local>size-1) std::cout<<"i_local "<<i_local<<" size "<<size<<std::endl;
        ...
        for (int l = base_level; l < domain_->tree()->depth(); l++)
        {
            for (auto it = domain_->begin(l); it != domain_->end(l); ++it)
            {
                     ...
                     field_idx < idx_u_type::nFields(); ++field_idx)
                {
                    for (auto& n : it->data())
                    {
                        auto cur_idx = n(idx_u, field_idx);
                        if (cur_idx > 0)
                        {
                            if (min_local_idx == -1)
                            {
                                min_local_idx = cur_idx;
                            }
                            else if (cur_idx > max_local_idx)
                            {
                                max_local_idx = cur_idx;
                            }
                            else if (cur_idx < min_local_idx)
                            {
                                min_local_idx = cur_idx;
                            }
                        }
             ...
             l < domain_->tree()->depth(); ++l)
        {
            ...
            for (auto it = domain_->begin(l); it != domain_->end(l); ++it)
            {
                ...
                for (auto& n : it->data())
                {
                    ...
                    float_type avg_u =
                        n(Source::tag(), 0) / 2.0 +
                        n.at_offset(Source::tag(), 0, -1, 0) / 2.0;
                    float_type avg_v =
                        n(Source::tag(), 1) / 2.0 +
                        n.at_offset(Source::tag(), -1, 0, 1) / 2.0;

                    float_type u_theta =
                        (x * avg_v - y * avg_u) / std::sqrt(x * x + y * y);
                    float_type u_r =
                        (x * avg_u + y * avg_v) / std::sqrt(x * x + y * y);
        ...
        std::vector<key_t>& octs, std::vector<int>& level_change)
    {
        ...
        for (auto it = domain_->begin(); it != domain_->end(); ++it)
        {
            ...
            if (!it->is_correction() && it->is_leaf())
                l1 = this->template adapt_levle_change_for_field<cell_aux_type>(
                    it, source_max[0], true);

            if (it->is_correction() && !it->is_leaf())
                l2 = this->template adapt_levle_change_for_field<
                    correction_tmp_type>(it, source_max[0], false);

            if (!it->is_correction() && it->is_leaf())
                l3 = this->template adapt_levle_change_for_field<edge_aux_type>(
                    it, source_max[1], false);
    ...
    int adapt_level_change(OctantType* it, std::vector<float_type> source_max)
    {
        ...
        if (it->is_correction()) return -1;

        int l1 = this->template adapt_levle_change_for_field<cell_aux>(it,
            source_max[0], true);
        //int l1 = -1;
    ...
    int adapt_levle_change_for_field(OctantType it, float_type source_max,
        bool use_base_level_threshold)
    {
        ...
        float_type field_max = domain::Operator::maxnorm<Field>(it->data());
        ...
        int l_delete_aim = static_cast<int>(
            ceil(nLevelRefinement_ -
                 ((log(field_max / source_max) - log(deletion_factor)) /
                     log(refinement_factor_))));

        if (l_aim > hard_max_level_) l_aim = hard_max_level_;
    ...
    template<class Field, class OctantType>
    int adapt_levle_change_for_field_old(OctantType it, float_type source_max,
        bool use_base_level_threshold)
    {
        ...
        float_type field_max = domain::Operator::maxabs<Field>(it->data());
        ...
        if ((l_delete_aim < 0) || (!use_base_level_threshold && l_aim == 0))
            return -1;
        if (l_change > 0) return 1;
        return 0;
    }
    ...
    float_type vortex_ring_vor_fat_ic(float_type x, float_type y, int field_idx,
        bool perturbation)
    {
        //return 1;
    ...
    float_type vor(float_type x, float_type y, int field_idx) const
    {
        ...
        return vr_fct_(x_loc, y_loc, field_idx,
            perturbation_) /* - vr_fct_(x_loc,y_loc-vort_sep,field_idx,perturbation_))*/
            ;
        //else
        //return -vr_fct_(x,y,z-d2v_/2,field_idx,perturbation_)+vr_fct_(x,y,z+d2v_/2,field_idx,perturbation_);
    }

    float_type vortex_ring_vor_ic(float_type x, float_type y, int field_idx,
        bool perturbation)
    {
        //return 1;
        //float_type gam = 1.0;
        ...
        bool use_all = false) const noexcept
    {
        ...
        float_type w_max = std::abs(
            vr_fct_(float_type(0.0), float_type(0.0), 0, perturbation_));
        ...
        for (int i = b.base()[0]; i <= b.max()[0]; ++i)
        {
            for (int j = b.base()[1]; j <= b.max()[1]; ++j)
            {
                ...
                float_type half_block =
                    static_cast<float_type>(b.extent()[0]) * dx_level / 2.0;
     ...
     */
    std::vector<coordinate_t> initialize_domain(Dictionary* _d,
        domain_t*                                           _domain)
    {
        auto res =
    ...
    float_type u_vort(float_type x, float_type y, float_type td, int idx = 0)
    {
        float_type x_loc = x - ctr_dis_x;
        float_type y_loc = y - ctr_dis_y;
        if (vortexType == 1) { return u_taylor_vort(x_loc, y_loc, td, idx); }
        else if (vortexType == 2)
        {
            return u_oseen_vort(x_loc, y_loc, td, idx);
        }
        else if (vortexType == 0) { return 0.0; }
        else { return 0.0; }
    }
    ...
    float_type w_taylor_vort(float_type rd, float_type td)
    {
        ...
        float_type w = 2.0 * (t_0 / t) * (t_0 / t) / R_ *
                       (1.0 - r2 / 2.0 / R_ / R_ * (t_0 / t)) * expval;
        //float_type w = gam/4.0/M_PI*Re_/t*std::exp(-r2/4.0*Re_/t);
        //float_type w = 1.0/4.0/M_PI/nu/t/t*(1.0 - r2/4.0/nu/t)*std::exp(-r2/4.0/nu/t);
        return w;
    }

    float_type u_taylor_vort(float_type x, float_type y, float_type td,
        int idx = 0)
    {
        float_type t_0 = Re_ / 2.0 / R_ / R_;
        ...
        if (idx == 0) { multiplier = -std::sin(theta); }
        else if (idx == 1) { multiplier = std::cos(theta); }
    ...
    {
        float_type mean_c =
            2.24181; //if using non-dim in Panton, max vel happens at eta = 2.24181
        float_type fac =
            2.0 * mean_c * mean_c /
            (mean_c * mean_c + 2); //factor to make maxvelocity to be 1
    ...
    float_type u_oseen_vort(float_type x, float_type y, float_type td,
        int idx = 0)
    {
        float_type mean_c =
            2.24181; //if using non-dim in Panton, max vel happens at eta = 2.24181
  ...
  private:
    ...
    // time_integration_t ifherk(&simulation_);
    std::shared_ptr<time_integration_t> ifherk_;
    ...
    float_type base_threshold_deletion_ = 1e-4;
    vr_fct_t   vr_fct_;
