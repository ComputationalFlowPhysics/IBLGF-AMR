...
def generate_configs(sample_config, output_dir, *, prefix):
    ...
    for updates in parameter_sets:
        ...
        config_path = output_dir / "{}_freq{}.cfg".format(prefix, filename_number(freq_value))
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate ns_amr_lgf (3D) configs for a b_f_freq sweep."
    )
    parser.add_argument(
        "sample_config",
        type=Path,
        help="Sample 3D config file to copy from.",
    )
    parser.add_argument(
        "-o",
        "--output-dir",
    ...
    for updates, path in generated:
        print("{} b_f_freq={}".format(path.name, format_number(updates["b_f_freq"])))
