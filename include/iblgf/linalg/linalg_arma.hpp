...
class L_armadillo
{
  public:
    ...
    static auto cube_wrap(
        types::float_type* ptr_aux_mem, int n_rows, int n_cols, int n_slices)
    {
    ...
    static auto submatrix(
        mat_t& data_, std::tuple<int, int> nx, std::tuple<int, int> ny)
    {
        return data_(arma::span(std::get<0>(nx), std::get<1>(nx)),
            arma::span(std::get<0>(ny), std::get<1>(ny)));
    }
