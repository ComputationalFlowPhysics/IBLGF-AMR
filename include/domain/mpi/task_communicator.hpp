...
: public crtp::Crtps<Derived, TaskCommunicator<TaskType, Derived>>
{
  ...
  public: //Ctor
    TaskCommunicator() { buffer_.init(1000); }
  ...
  public:
    ...
    task_ptr_t insert(
        const id_type& _taskId, task_data_t* _data, int _rank_other) noexcept
    {
        //++nActive_tasks_;
    ...
    {
        return pack_messages_
                   ? acc_comm()->done()
                   : (tasks_.size() == 0 && buffer_queue_.size() == 0 &&
  ...
  protected:
    ...
    void insert_unconfirmed_tasks(task_ptr_t& t) noexcept
    {
        if (t->requires_confirmation())
        {
            ...
            if (!confirmed) { unconfirmed_tasks_.push_back(t); }
        }
    }
    void check_unconfirmed_tasks() noexcept
    {
        for (auto it = unconfirmed_tasks_.begin();
             it != unconfirmed_tasks_.end();)
        {
            if ((*it)->confirmed()) { it = unconfirmed_tasks_.erase(it); }
            else
            {
                ++it;
            }
...
: public TaskCommunicator<TaskType, SendTaskCommunicator<TaskType>>
{
  public:
    using super_type =
        TaskCommunicator<TaskType, SendTaskCommunicator<TaskType>>;
  ...
  public: //Memebers:
    ...
    void to_buffer_impl(task_ptr_t _t) noexcept { _t->assign_data2buffer(); }
    ...
    void pack_masseges_impl(std::size_t rank_other) noexcept
    {
        ...
        for (std::size_t i = 0; i < this->acc_tasks[rank_other].size(); ++i)
        {
            ...
            this->acc_fields[rank_other].insert(
                this->acc_fields[rank_other].end(), task->send_buffer().begin(),
...
: public TaskCommunicator<TaskType, RecvTaskCommunicator<TaskType>>
{
  public:
    using super_type =
        TaskCommunicator<TaskType, RecvTaskCommunicator<TaskType>>;
    using task_ptr_t = typename super_type::task_ptr_t;
  ...
  public: //members
    ...
    {
        if (this->comm_.iprobe(_t->rank_other(), _t->id())) return true;
        else
            return false;
    }

    void pack_masseges_impl(std::size_t rank_other) noexcept { return; }
