...
class extrapolation_cell_center_nli
{
  ...
  public: // constructor
    extrapolation_cell_center_nli() = delete;
    extrapolation_cell_center_nli(size_t Nb_)
    : Nb_(Nb_)
          ...
          xt::xtensor<float_type, 2>(std::array<size_t, 2>{{Nb_, Nb_}})}
    {
        ...
        xt::noalias(antrp_mat_sub_simple_sub_[1]) =
            view(antrp_mat_sub_simple_[0], xt::all(),
                xt::range(Nb_ - 2, 2 * Nb_ - 2));
        ...
        xt::noalias(antrp_mat_sub_simple_sub_[3]) =
            view(antrp_mat_sub_simple_[1], xt::all(),
                xt::range(Nb_ - 2, 2 * Nb_ - 2));
  ...
  public: // functionalities
        ...
        int child_idx, MeshObject mesh_obj, std::size_t _field_idx)
    {
        ...
        int idx_z = (child_idx & (1 << 2)) >> 2;
        xt::noalias(
            view(child, (1 - idx_x) * (Nb_ - 1), xt::all(), xt::all())) *= 0.0;
        xt::noalias(
            view(child, xt::all(), (1 - idx_y) * (Nb_ - 1), xt::all())) *= 0.0;
        xt::noalias(
            view(child, xt::all(), xt::all(), (1 - idx_z) * (Nb_ - 1))) *= 0.0;
  ...
  private:
        ...
        linalg_data_t& antrp_mat_relative_pos_1_, int Nb_)
    {
        for (int c = 1; c < Nb_ * 2 - 1; ++c)
        {
            ...
            if (c_p < -1)
            {
                bd_l = std::max(1, ((c + 1) / 2 - pts_cap / 2));
                bd_r = std::min(Nb_ - 1, bd_l + pts_cap);
            }
            else
            {
                bd_r = std::min(Nb_ - 1, ((c + 1) / 2 + pts_cap / 2));
                bd_l = std::max(1, bd_r - pts_cap);
            }
            ...
            for (int p = bd_l; p < bd_r; ++p)
            {
                ...
                for (int l = bd_l; l < bd_r; ++l)
                {
                    ...
                    {
                        temp_mult /= -(long double)(2 * l - Nb_ + 1 - p_p);
                        //temp_mult *= -(long double)(2*l - Nb_ + 1 - c_p);
                    }
                }
