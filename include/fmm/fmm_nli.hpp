  ...
  private:
    ...
    void antrp_mat_calc_fourier(linalg_data_t& antrp_mat_, int Nb_)
    {
        ...
        xt::xtensor<std::complex<double>, 2> mat_cal_basis(
            std::array<size_t, 2>{
                {static_cast<size_t>(Nb_), static_cast<size_t>(M)}});
        xt::xtensor<std::complex<double>, 2> mat_cal_intrp(
            std::array<size_t, 2>{
                {static_cast<size_t>(2 * Nb_ - 1), static_cast<size_t>(M)}});
        ...
        auto tmp = xt::linalg::dot(mat_cal_intrp, xt::linalg::dot(wtw_inv, wt));
