...
struct subdivision_generator
{
                      ...
                      std::vector<BlockDescriptor>& blocks,int level)
    {
        ...
        {
            subdivision_generator<ND-1,Dim, BlockDescriptor>::
                apply_impl(cuts, idx, blocks, level);
            ++idx[ND-1];
            idx[ND-2] = 0;
        }
...
public:  
                               ...
                               int _level=0 ) 
    {
        ...
        detail::
            subdivision_generator<dimension(),
                                  dimension(),
...
public: //members
    ...
    std::vector<BlockDescriptor> cutout(const std::vector<Block>& _blocks) const noexcept
    {
        ...
        detail::
            subdivision_generator<dim,dim,Block>::apply(cut_locations,res,level_);
...
private:
            ...
            ) const
    {
        ...
        {
            auto tmp= get_periodic_boxes(_block,_periodicityBlock,
                        _periodicty,_dimension+1);

            res.insert(res.end(),tmp.begin(),tmp.end());
        }
        else res.push_back(_block);
        ...
        if(_dimension<nDims-1 )
        {
            auto tmp= get_periodic_boxes(_block,
                    _periodicityBlock,_periodicty,_dimension+1);
            res.insert(res.end(),tmp.begin(),tmp.end());
        }
        else res.push_back(_block);
