...
class ib_communicator
{
  ...
  public: // init functions
     ...
     * */
    void start_communication(bool send_locally_owned)
    {
        pack_messages(send_locally_owned);
     ...
     * */
    void communicate(bool send_locally_owned)
    {
        start_communication(send_locally_owned);
        finish_communication(send_locally_owned);
    }

  private: //Private member: Packing/Unpacking
    /**@brief Pack all messages to the send buffer.
     * */
    void pack_messages(bool send_locally_owned) noexcept
    {
        //send
        auto& indices =
            send_locally_owned ? locally_owned_indices_ : ghost_indices_;
        ...
        for (std::size_t rank = 0; rank < indices.size(); ++rank)
        {
            ...
            { data[rank][i] = ib_->force(indices[rank][i]); }
        }
    }
     ...
     * added to the forces.
     * */
    void unpack_messages(bool send_locally_owned)
    {
        auto& indices =
            send_locally_owned ? ghost_indices_ : locally_owned_indices_;
        ...
        for (std::size_t rank = 0; rank < indices.size(); ++rank)
        {
            ...
            for (std::size_t i = 0; i < indices[rank].size(); ++i)
            {
                ib_->force(indices[rank][i]) =
                    send_locally_owned
                        ? data[rank][i]
                        : ib_->force(indices[rank][i]) + data[rank][i];
            }
        }
    }
