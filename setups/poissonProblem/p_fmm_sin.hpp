...
struct p_fmm_sin
{
    ...
    make_field_type(phi_num, float_type, 1, 1)
        ...
        make_field_type(amr_lap_source, float_type, 1, 1)
            make_field_type(amr_lap_tmp, float_type, 1, 1)
                ...
                using datablock_t = DataBlock<Dim, node, phi_num, source,
                    phi_exact, error, fmm_s_type, fmm_t_type, fmm_tmp,
                    phi_num_fmm, amr_lap_source, amr_lap_tmp, error_lap_source>;
    ...
    void initialize()
    {
        ...
        for (int i = 0; i < b_max_0; ++i)
            for (int j = 0; j < b_max_1; ++j)
                ...
                {
                    float_type xx =
                        (2 * M_PI * wave_number * i) / (b_max_0 - 1) -
                        M_PI * wave_number;
                    float_type yy =
                        (2 * M_PI * wave_number * j) / (b_max_1 - 1) -
                        M_PI * wave_number;
                    float_type zz =
                        (2 * M_PI * wave_number * k) / (b_max_2 - 1) -
                        M_PI * wave_number;

                    soln(i, j, k) = sin(xx) * sin(yy) * sin(zz) *
                                    exp(-alpha * (xx * xx + yy * yy + zz * zz));
                }
    ...
    void run()
    {
        ...
        psolver.solve<source, phi_num, fmm_s_type, fmm_t_type, phi_num_fmm,
            fmm_tmp, amr_lap_source, amr_lap_tmp>();
        compute_errors();
