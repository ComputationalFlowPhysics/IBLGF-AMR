...
def generate_configs(sample_config, output_dir, *, prefix):
    ...
    for updates in parameter_sets:
        ...
        config_path = output_dir / "{}_tau{}.cfg".format(prefix, filename_number(tau_value))
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate ns_amr_lgf (3D) configs for a b_f_tau sweep."
    )
    parser.add_argument(
        "sample_config",
        type=Path,
    ...
    for updates, path in generated:
        print("{} b_f_tau={}".format(path.name, format_number(updates["b_f_tau"])))
