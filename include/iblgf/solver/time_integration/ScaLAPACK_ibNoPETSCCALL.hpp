...
class DirectIB {
	...
	std::vector<force_type> mat = mat_;
        ...
        for (int ModeNum = 0; ModeNum < localModes.size(); ModeNum++) {
            ...
            MatCreateScaLAPACK(PETSC_COMM_WORLD,PETSC_DECIDE , PETSC_DECIDE , n, n, 0,0, &A[ModeNum]);
            MatSetFromOptions(A[ModeNum]);
            MatSetUp(A[ModeNum]);

            MatCreateVecs(A[ModeNum],&x[ModeNum],&b[ModeNum]);

            
            VecDuplicate(x[ModeNum], &u[ModeNum]);

            VecGetOwnershipRange(x[ModeNum], &rstartx[ModeNum], &rendx[ModeNum]);

            VecGetOwnershipRange(b[ModeNum], &rstartb[ModeNum], &rendb[ModeNum]);
            ...
            for (int i = rstartb[ModeNum]; i < rendb[ModeNum]; i++)
            {
                ...
                for (int j = 0; j < n; j++)
                {
                    ...
                    MatSetValues(A[ModeNum], 1, &i, 1, &j, &valA,
                        INSERT_VALUES);
                }
            }
            ...
            KSPCreate(PETSC_COMM_WORLD, &ksp[ModeNum]);
            KSPSetOperators(ksp[ModeNum], A[ModeNum], A[ModeNum]);

            KSPGetPC(ksp[ModeNum], &pc[ModeNum]);
            PCSetType(pc[ModeNum], PCLU);

            KSPSetTolerances(ksp[ModeNum], 1.e-7, PETSC_DEFAULT, PETSC_DEFAULT,
                PETSC_DEFAULT);

            KSPSetFromOptions(ksp[ModeNum]);
        }
        return 0;
    }
    ...
    int final_assemble()
    {
        ...
        for (int ModeNum = 0; ModeNum < localModes.size(); ModeNum++)
        {
            ...
            KSPSetTolerances(ksp[ModeNum], 1.e-7, PETSC_DEFAULT,
    ...
    int load_RHS(force_type& forcing, bool summed = true) {
        ...
        for (int ModeNum = 0; ModeNum < localModes.size(); ModeNum++) {
            ...
            for (int i = rstartb[ModeNum]; i < rendb[ModeNum]; i++)
            {
                ...
                VecSetValues(b[ModeNum], 1, &i, &v, INSERT_VALUES);
            }
    ...
    int getSolution(force_type& res) {
        ...
        for (int ModeNum = 0; ModeNum < localModes.size(); ModeNum++) {
            ...
            KSPSolve(ksp[ModeNum], b[ModeNum], x[ModeNum]);
