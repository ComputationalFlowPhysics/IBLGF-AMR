...
struct NS_AMR_LGF : public SetupNewton<NS_AMR_LGF, parameters>
{
	...
	float_type run()
	{
        ...
        if (world.rank() == 1) {
            std::cout << "including zero size is " << ifherk.mat.tot_size(true) << std::endl;
            std::cout << "not including zero size is " << ifherk.mat.tot_size(false) << std::endl;
        }
        ...
        {
            iparm[40] =
                1; /* The number of row in global matrix, rhs element and solution vector
                          that begins the input domain belonging to this MPI process */
            iparm[41] =
                3; /* The number of row in global matrix, rhs element and solution vector
            ...
            if (rank == 1)
            {
                iparm[40] =
                    3; /* The number of row in global matrix, rhs element and solution vector
                              that begins the input domain belonging to this MPI process*/
                iparm[41] =
                    5; /* The number of row in global matrix, rhs element and solution vector
                              that ends the input domain belonging to this MPI process*/
                ia = (MKL_INT*)MKL_malloc(sizeof(MKL_INT) * 4, 64);
                ...
                x = (double*)MKL_malloc(sizeof(double) * 3, 64);
                b = (double*)MKL_malloc(sizeof(double) * 3, 64);
                if (ia == NULL || ja == NULL || a == NULL || x == NULL ||
                    b == NULL)
                {
                    err_mem = 1;
