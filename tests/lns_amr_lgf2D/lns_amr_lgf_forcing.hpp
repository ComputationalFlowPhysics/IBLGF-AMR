...
struct NS_AMR_LGF : public SetupLinear<NS_AMR_LGF, parameters>
{
    ...
    : super_type(_d, [this](auto _d, auto _domain) { return this->initialize_domain(_d, _domain); })
    {
        ...
        simulation_.bc_vel() = [this](std::size_t idx, float_type t, auto coord = {0, 0})
        ...
        simulation_.frame_vel() = [this](std::size_t idx, float_type t, auto coord = {0, 0})
        {
            float_type T0 = 0.5;
        ...
        if (use_fat_ring)
            vr_fct_ = [this](float_type x, float_type y, int field_idx, bool perturbation)
            { return this->vortex_ring_vor_fat_ic(x, y, field_idx, perturbation); };
        else
            vr_fct_ = [this](float_type x, float_type y, int field_idx, bool perturbation)
            { return this->vortex_ring_vor_ic(x, y, field_idx, perturbation); };

        ic_filename_ = simulation_.dictionary_->template get_or<std::string>("hdf5_ic_name", "null");

        ref_filename_ = simulation_.dictionary_->template get_or<std::string>("hdf5_ref_name", "null");

        source_max_ = simulation_.dictionary_->template get_or<float_type>("source_max", 1.0);

        refinement_factor_ = simulation_.dictionary_->template get<float_type>("refinement_factor");

        base_threshold_ = simulation_.dictionary()->template get_or<float_type>("base_level_threshold", 1e-4);
        ...
        if (vortexType != 0)
        {
            domain_->register_refinement_condition() = [this](auto octant, int diff_level)
    ...
    float_type run()
    {
        ...
        if (ref_filename_ != "null")
        {
            ...
            auto center = (domain_->bounding_box().max() - domain_->bounding_box().min() + 1) / 2.0 +
            ...
            for (auto it = domain_->begin_leaves(); it != domain_->end_leaves(); ++it)
            {
                ...
                for (auto& node : it->data())
                {
                    ...
                    float_type  y = static_cast<float_type>(coord[1] - center[1] * scaling) * dx_level;
                    ...
                    node(w_num) =
                        (node(u, 1) - node.at_offset(u, -1, 0, 1) - node(u, 0) + node.at_offset(u, 0, -1, 0)) /
                        dx_level;
                    if (vortexType != 0)
                    {
        ...
        float_type u1_inf = this->compute_errors<u_type, u_ref_type, error_u_type>(std::string("u1_"), 0);
        float_type u2_inf = this->compute_errors<u_type, u_ref_type, error_u_type>(std::string("u2_"), 1);
        float_type u_t_inf =
            this->compute_errors<num_u_theta_type, exact_u_theta_type, error_u_theta_type>(std::string("u_t_"), 0);
        float_type u_r_inf =
            this->compute_errors<num_u_theta_type, exact_u_theta_type, error_u_theta_type>(std::string("u_r_"), 1);
    ...
    template<class Source, class Target1, class Target2>
    void test_buffer_exchange(time_integration_t* ifherk) noexcept
    {
        boost::mpi::communicator world;
        if (domain_->is_server()) return;
        auto center =
            (domain_->bounding_box().max() - domain_->bounding_box().min() + 1) / 2.0 + domain_->bounding_box().min();
        // //set leafs value to refinement level
        // for( auto it=domain_->begin_leaves();it!=domain_->end_leaves(); ++it)
        // {
        //     if(!it->locally_owned()) continue;
        //     auto ref_level=it->refinement_level();
        //     auto dx_level = domain_->dx_base() / std::pow(2, it->refinement_level());
        //     auto scaling = std::pow(2, it->refinement_level());
        //     for (auto& node : it->data())
        //     {
        //         node(Target::tag(), 0) = ref_level;
        //     }

        // }
        ifherk->up_and_down<Source>();
        for (int l = domain_->tree()->base_level(); l < domain_->tree()->depth(); ++l)
        {
            ...
            for (auto it = domain_->begin(l); it != domain_->end(l); ++it)
            {
                ...
                for(auto& node:it->data())
                {
                    ...
                    node(Target1::tag(), 0)=node.at_offset(Source::tag(), 0, -1, 0);
                    node(Target1::tag(), 1)=node.at_offset(Source::tag(), -1, 0, 1);
                    node(Target2::tag(), 0)=node(Source::tag(), 0)-node.at_offset(Source::tag(), 0, -1, 0);
                    node(Target2::tag(), 1)=node(Source::tag(), 1)-node.at_offset(Source::tag(), -1, 0, 1);
                    node(Target2::tag(), 0)*=fac;
                    node(Target2::tag(), 1)*=fac;
    ...
    void adapt_level_change(std::vector<float_type> source_max, std::vector<key_t>& octs,
    ...
    int adapt_levle_change_for_field(OctantType it, float_type source_max, bool use_base_level_threshold)
    {
        ...
        if (it->is_ib() && it->is_leaf())
            if (it->refinement_level() < nLevelRefinement_ + nIB_add_level_) return 1;
            else
                return 0;
        ...
        int l_delete_aim = static_cast<int>(
            ceil(nLevelRefinement_ - ((log(field_max / source_max) - log(deletion_factor)) / log(refinement_factor_))));
        ...
        {
            if (field_max > source_max * base_threshold_) l_aim = std::max(l_aim, 0);

            if (field_max > source_max * base_threshold_ * deletion_factor) l_delete_aim = std::max(l_delete_aim, 0);
        }
    ...
    int adapt_levle_change_for_field_old(OctantType it, float_type source_max, bool use_base_level_threshold)
    {
        ...
        int l_aim = static_cast<int>(ceil(nLevelRefinement_ - log(field_max / source_max) / log(refinement_factor_)));
        int l_delete_aim =
            static_cast<int>(ceil(nLevelRefinement_ - log(field_max / source_max) / log(deletion_factor)));

        if (hard_max_refinement_ && (l_aim > nLevelRefinement_)) l_aim = nLevelRefinement_;
    ...
    float_type u_vort(float_type x, float_type y, float_type td, int idx = 0)
    {
        ...
        else if (vortexType == 2) { return u_oseen_vort(x_loc, y_loc, td, idx); }
