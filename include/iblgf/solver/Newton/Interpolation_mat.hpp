...
#ifndef IBLGF_INCLUDED_C_NLI_MAT
#define IBLGF_INCLUDED_C_NLI_MAT
...
class cell_nli_mat
{
    ...
    public: // constructor
        ...
        cell_nli_mat() = delete;
        cell_nli_mat(size_t Nb_, int intrp_order)
            : Nb_(Nb_),
        ...
        for (int c = 0; c < Nb_ * 2; ++c)
        {
            ...
            else
            {
                ...
                for (int p = bd_l; p < bd_r; ++p)
                {
                    ...
                    antrp_mat_(p, c) = (double)temp_mult;
                    temp_sum += temp_mult;
                }
                view(antrp_mat_, xt::all(), c) /= (double)temp_sum;
            }
        }
    }
        ...
        linalg_data_t& antrp_mat_relative_pos_1_, int Nb_)
    {
        for (int c = 0; c < Nb_ * 2; ++c)
        {
            ...
            for (int p = bd_l; p < bd_r; ++p)
            {
                ...
                antrp_mat_relative_pos_1_(p, c) = (double)temp_mult;
                temp_sum += temp_mult;
            }
