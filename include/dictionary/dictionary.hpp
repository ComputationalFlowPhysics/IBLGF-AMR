...
public:
    DictionaryExcpetion(std::string _name,std::string _key) : 
        std::runtime_error("dictionary " + _name + ": key " + _key + " not found!") 
    {}
...
private: //Helper functions
    ...
    void init()
    {
        auto dicts=string_utilities::get_dictionaries(content_,opening_delimiter_,closing_delimiter_);
        string_utilities::erase_dictionaries(content_,opening_delimiter_,closing_delimiter_);
        for(auto& d: dicts)
        {
           sub_dictionaries_.emplace_back(std::make_shared<Dictionary>(d.first, d.second));
...
public: //Some access functions
    ...
    {
		auto it = std::find_if(sub_dictionaries_.cbegin(), sub_dictionaries_.cend(), 
...
public: //Access functions
	...
	bool has_key(const std::string& key) const
	{
		...
		else return (vector_variables_.find(key) != vector_variables_.cend());
	}
    ...
    {
       try { return get<T>(_key); }
       catch(...) { return _default; } 
    }
    template<typename T,std::size_t N>
    auto get_or(const std::string& _key, const vector_type<T,N>& _default) const 
    {
       try { return get<T,N>(_key); }
       catch(...) { return _default; } 
    }
    ...
    std::ostream& print(std::ostream& os,std::string _tab="") const
    {
        if(!root)
        {
            ...
            {
                os<<_tab<<'\t'<<e.first<<"="<<e.second<<";"<<std::endl;
            }
