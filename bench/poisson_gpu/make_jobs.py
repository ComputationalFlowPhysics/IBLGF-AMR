...
# CONFIGURATION
nodes_to_test = [4,5,6,8,10]   # Total physical nodes
gpus_per_node = 4        # Delta standard 
bench_dir = os.getcwd()

# Define absolute paths to ensure the srun finds them from subfolders
app_path = os.path.abspath("my-app_gpu.sif")
exe_path = os.path.abspath("poisson_gpu.x")
...
for nnodes in nodes_to_test:
    ...
    with open(job_filename, "w") as job_file:
        ...
        for n in n_vec:
            for block_power in n_vec[n]:
                ...
                app_cmd = f"apptainer exec --nv --bind {bench_dir} {app_path} {exe_path} ../config.cf"
    ...
    with open(sbatch_filename, "w") as sbatch_file:
        ...
        sbatch_file.write("#SBATCH --account=bfzi-delta-gpu\n")
        sbatch_file.write("#SBATCH --partition=gpuA40x4\n")
        sbatch_file.write(f"#SBATCH --nodes={nnodes}\n")
        sbatch_file.write(f"#SBATCH --gpus-per-node={gpus_per_node}\n")
        sbatch_file.write(f"#SBATCH --tasks-per-node={gpus_per_node + 1}\n") # High count to handle the G+1 task
