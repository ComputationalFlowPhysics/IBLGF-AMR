...
struct p_fmm_rndm
{
    ...
    make_field_type(phi_num, float_type, 1, 1)
        ...
        make_field_type(fmm_s_type, float_type, 1, 1) make_field_type(
            fmm_t_type, float_type, 1, 1) make_field_type(fmm_tmp, float_type,
            1, 1) make_field_type(phi_num_fmm, float_type, 1, 1)

            using datablock_t = DataBlock<Dim, node, phi_num, source, phi_exact,
                error, fmm_s_type, fmm_t_type, fmm_tmp, phi_num_fmm>;
    ...
    void initialize()
    {
        ...
        xt::xtensor<float_type, 3> rndm_soln =
            xt::random::rand<float_type>({b_max_0, b_max_1, b_max_2}) * 2 - 1;
        xt::xtensor<float_type, 3> f_source(
            std::array<size_t, 3>{{b_max_0, b_max_1, b_max_2}});
        ...
        for (int i = 0; i < b_max_0; ++i)
            for (int j = 0; j < b_max_1; ++j)
                for (int k = 0; k < b_max_2; ++k)
                {
                    ...
                    auto kk = k;
                    if ((ii >= 0 && ii < b_max_0) &&
                        (jj >= 0 && jj < b_max_1) && (kk >= 0 && kk < b_max_2))
        ...
        for (auto it = domain_->begin_leafs(); it != domain_->end_leafs(); ++it)
        {
                 ...
                 ++it2)
            {
                ...
                it2->get<source>() = f_source(coord[0], coord[1], coord[2]) /
                                     (dx_level * dx_level);
        ...
        for (auto it = domain_->begin_leafs(); it != domain_->end_leafs(); ++it)
        { auto a = it->data_ref().get_linalg_data<phi_exact>(); }
    }
    ...
    void run()
    {
        ...
        psolver.solve<source, phi_num, fmm_s_type, fmm_t_type, phi_num_fmm,
            fmm_tmp>();
        compute_errors();
