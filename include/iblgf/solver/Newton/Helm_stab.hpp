...
class Helm_stab
{
  ...
  public:
                 ...
                 it != domain_->end(l); ++it)
            {
                ...
                else //if (it->is_correction())
                {
                    ...
                    for (int i = 0; i < it->num_neighbors(); i++)
                    {
                        ...
                        {
                            if (i == 0) { tmp[0][0] = true; tmp[1][0] = true;
                                          tmp[0][1] = true; tmp[1][1] = true;}
                            if (i == 1)
                            {
                                for (int j = 0; j < N; j++)
                                {
                            ...
                            if (i == 2) { tmp[N - 1][0] = true; tmp[N - 1][1] = true;
                                          tmp[N - 2][0] = true; tmp[N - 2][1] = true;}
                            if (i == 3)
                            {
                                for (int j = 0; j < N; j++)
                                {
                            ...
                            if (i == 6) { tmp[0][N - 1] = true; tmp[1][N - 1] = true;
                                          tmp[0][N - 2] = true; tmp[1][N - 2] = true;}
                            if (i == 7)
                            {
                                for (int j = 0; j < N; j++)
                                {
                            ...
                            if (i == 8) { tmp[N - 1][N - 1] = true; tmp[N - 2][N - 1] = true;
                                          tmp[N - 1][N - 2] = true; tmp[N - 2][N - 2] = true;}
                        }
                    }
                    for (std::size_t field_idx = 0;
                         field_idx < idx_u_type::nFields(); ++field_idx)
                    ...
                    {
                        auto& lin_data =
                            it->data_r(idx_u_B_type::tag(), field_idx)
                                .linalg_data();
                        for (int i = 0; i < N; i++)
                        {
                            for (int j = 0; j < N; j++)
        ...
        for (int l = base_level; l < domain_->tree()->depth(); l++)
        {
                 ...
                 it != domain_->end(l); ++it)
            {
                ...
                /*for (auto& n : it->data())
                {
                    ...
                    if (cur_idx > 0) { IV.add_element(cur_idx, glo_idx, 1.0); }
    ...
    void construct_filter_matrices() {
    ...
    void construction_B_i_matrix() {
        ...
        if (world.rank() == 1) {
            std::cout << "Constructing B_i matrix" << std::endl;
        }
        ...
        B_i.resizing_row(max_local_B_idx+1);
                 ...
                 it != domain_->end(l); ++it)
            {
                     ...
                     field_idx < idx_u_type::nFields(); ++field_idx)
                {
                    for (auto& n : it->data())
                    {
                        ...
                        int glo_idx = n(idx_u_g_type::tag(), field_idx);
                        if (cur_idx> 0) {
                            B_i.add_element(cur_idx, glo_idx, 1.0);
                        }
                    }
                }
                ...
                /*for (auto& n : it->data())
                {
                    ...
                    int cur_idx = n(idx_uz_B_type::tag(), 0);
                    int glo_idx = n(idx_uz_g_type::tag(), 0);
    ...
    void div_access(bool iscs = true) {
        this->divergence<Source, Target>();
        add<Uz, Target>(-mode_c);
        if (iscs) clean_leaf_according_to_idx<Target, idx_cs_g_type>(domain_->tree()->base_level());
    }
    ...
    template<class Source, class Uz, class Target>
    void curlTranspose() noexcept
    {
        auto client = domain_->decomposition().client();

        domain::Operator::domainClean<Target>(domain_);
        domain::Operator::domainClean<Uz>(domain_);
                 ...
                 l < domain_->tree()->depth(); ++l)
        {
            ...
            for (auto it = domain_->begin(l); it != domain_->end(l); ++it)
            {
                ...
                const auto dx_level =
                    dx_base_ / math::pow2(it->refinement_level());
                domain::Operator::curl_transpose_helmholtz_complex_linear<Source, Target, Uz>(
                    it->data(), dx_level, mode_c, 1.0/Re_);
            }
            //client->template buffer_exchange<Field>(l);
        }
    ...
    void Grad_access(float_type _scale = 1.0) {
...
public:
    ...
    template<class U_old, class W_old>
    void construction_DN_forcing(float_type t = 5) {
        boost::mpi::communicator world;
        world.barrier();
...
private:
  ...
  public:
    ...
    sparse_mat ForcingMat_real; //Matrix defining the input of all forcings, this is defined N_grid_pts * N_forcing_pts to utilize sparse structure
  ...
  private:
    ...
    std::vector<real_coordinate_type> forcing_coord;
