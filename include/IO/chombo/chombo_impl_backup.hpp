...
public:
    ...
    // write global meta Data and structure of files.
                     ...
                     int _ref_ratio)
    {
        ...
        // iterate through field toget field names
        ...
        std::cout<<"Before broadcast: I am rank "<<world.rank()
                    <<" and numer of levels is "<<num_levels<<std::endl;

        world.barrier();
        boost::mpi::broadcast(world, num_levels, 0); // send from server to all others

        std::cout<<"After broadcast:  I am rank "<<world.rank()
                    <<" and number of levels is "<<num_levels<<std::endl;
        ...
        for (int lvl = 0; lvl < num_levels; ++lvl) {
            ...
            // Server:
            // Dataset size and patch offsets are collected by server
            // Needs to be sent to clients so dataset can be opened together
            // Dataset can be written just by server.
            //
            // Loop through blocks for

            std::vector<int> patch_offsets_vec;
            patch_offsets_vec.push_back(0);

            hsize_type dset_size = 0;   // Count size of dataset for this level
            hsize_type dset_size_patch_offsets = 0;   // Count size of dataset for this level

            // SERVER ONLY: Get dset_size and patch_offsets
            if (world.rank()==0) {
            ...
            // COLLECTIVELY: Create empty dataset (written later in parallel)
            auto space =_file->create_simple_space(1, &dset_size, NULL);
            auto dset_id=_file->template create_dataset<value_type>(group_id_lvl,
                "data:datatype=0", space);

            // COLLECTIVELY: Create + Write patch_offsets dataset
            dset_size_patch_offsets = patch_offsets_vec.size();
            auto space_patch_offsets = _file->create_simple_space(1,
                                        &dset_size_patch_offsets, NULL);
            auto dset_id_patch_offsets=_file->template create_dataset<int>(group_id_dattr,
                    "offsets",space_patch_offsets);

            // SERVER ONLY: Write patch_offsets (dataset), maybe move to write
            // data so server has something to do
            if (world.rank()==0) {
                _file->template write<int>(dset_id_patch_offsets,
                            dset_size_patch_offsets, &patch_offsets_vec[0]);
            }
...
//            _file->template write_boxCompound<Dim>(group_id, "boxes", mins, maxs, false);
            ...
            _file->close_space(space_patch_offsets);
            _file->close_dset(dset_id_patch_offsets);
            _file->close_dset(dset_id);
            _file->close_group(group_id_lvl);
        }
    ...
    //TODO: Write only data in parallel
    void write_level_data(HDF5File* _file)
    {
        boost::mpi::communicator world;

        std::cout<<"-> Write data with rank "<<world.rank()<<" of "<<world.size()<<std::endl;
        auto root = _file->get_root();
      ...
      //  offset_type offsets_test;
        ...
        for(auto& lp : level_map_)
        {
            ...
            // Open group lvl (already created)
...
//                                    std::cout<<field_value<<" "; //<<std::endl;
                ...
                if (world.rank()==1) {
                std::cout<<"Rank is "<<world.rank()<<". b is "<<b<<". Correct offset is "<<offset<<
