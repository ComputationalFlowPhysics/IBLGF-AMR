...
#ifndef IBLGF_INCLUDED_OPERATORJACTEST_HPP
#define IBLGF_INCLUDED_OPERATORJACTEST_HPP
...
struct parameters
{
    ...
    Dim,
        (
            ...
            (u_s,               float_type, 3, 1, 1, face, true),
            (u_p,               float_type, 3, 1, 1, face, true),
            (nl_jac_target,     float_type, 3, 1, 1, face, true),
            (nl_jac_exact,      float_type, 3, 1, 1, face, true),
            (nl_jac_error,      float_type, 3, 1, 1, face, true),
            (nl_jac_T_target,   float_type, 3, 1, 1, face, true),
            (nl_jac_T_exact,    float_type, 3, 1, 1, face, true),
            (nl_jac_T_error,    float_type, 3, 1, 1, face, true)
...
struct OperatorJacTest : public SetupLinear<OperatorJacTest, parameters>
{
    using super_type = SetupLinear<OperatorJacTest, parameters>;
    ...
    void run()
    {
        ...
        if (domain_->is_client())
        {
                 ...
                 ++it)
            {
                ...
                // domain::Operator::nonlinear_jacobian<u_s_type, u_p_type,
                    // nl_jac_target_type>(it->data(), dx_level);
                domain::Operator::curl<u_s_type,edge_aux_type>(it->data(), dx_level);
            }

            client->buffer_exchange<edge_aux_type>(base_level);
            for (auto it = domain_->begin_leaves(); it != domain_->end_leaves();
                 ++it)
            {
                if (!it->locally_owned() || !it->has_data()) continue;
                ...
                auto dx_level = dx_base / std::pow(2, it->refinement_level());
                domain::Operator::nonlinear<u_p_type,edge_aux_type,nl_jac_target_type>(it->data());

            }
            domain::Operator::domainClean<edge_aux_type>(domain_.get());
            for (auto it = domain_->begin_leaves(); it != domain_->end_leaves();
                 ++it)
            {
                if (!it->locally_owned() || !it->has_data()) continue;
                ...
                // domain::Operator::nonlinear_jacobian<u_s_type, u_p_type,
                    // nl_jac_target_type>(it->data(), dx_level);
                domain::Operator::curl<u_p_type,edge_aux_type>(it->data(), dx_level);
            }
                 ...
                 ++it)
            {
                ...
                auto dx_level = dx_base / std::pow(2, it->refinement_level());
                domain::Operator::nonlinear<u_s_type,edge_aux_type,face_aux_type>(it->data());

            }

            domain::Operator::add<face_aux_type,nl_jac_target_type>(domain_.get());

        }
        for (std::size_t field_idx=0; field_idx<Dim; ++field_idx)
        {
            this->compute_errors<nl_jac_target_type, nl_jac_exact_type,
            nl_jac_error_type>("nl_jac_", field_idx);
        }
        ...
        if (domain_->is_client())
        {
                 ...
                 ++it)
            {
                ...
                domain::Operator::nonlinear_adjoint_p1<u_s_type,u_p_type,
                    edge_aux_type>(it->data());
            }
            client->buffer_exchange<edge_aux_type>(base_level);
            for(auto it = domain_->begin_leaves(); it != domain_->end_leaves();
                 ++it)
            {
                ...
                domain::Operator::curl_transpose<edge_aux_type,nl_jac_T_target_type>(it->data(),dx_level);
            }
            domain::Operator::domainClean<edge_aux_type>(domain_.get());
            domain::Operator::domainClean<face_aux_type>(domain_.get());
            // domain::Operator::domainClean<nl_jac_T_target_type>(domain_.get());
            for(auto it = domain_->begin_leaves(); it != domain_->end_leaves();
                 ++it)
            {
                if (!it->locally_owned() || !it->has_data()) continue;
                 ...
                 ++it)
            {
                ...
                domain::Operator::nonlinear<u_p_type,edge_aux_type,face_aux_type>(it->data());
            }
            domain::Operator::add<face_aux_type,nl_jac_T_target_type>(domain_.get(),-1.0);
        }
        // this->compute_errors<nl_jac_T_target_type, nl_jac_T_exact_type,
        //     nl_jac_T_error_type>("nl_jac_T_");
        for (std::size_t field_idx=0; field_idx<Dim; ++field_idx)
        {
            this->compute_errors<nl_jac_T_target_type, nl_jac_T_exact_type,
            nl_jac_T_error_type>("nl_jac_T_", field_idx);
        }
    ...
    void initialize()
    {
             ...
             ++it)
        {
            ...
            for (auto& node : it->data())
            {
                ...
                // node(u_s,0)=tmpf0*yf0*zf0;
                // node(u_s,1)=tmpf0*xf1*zf1;
                // node(u_s,2)=tmpf0*xf2*yf2;

                // node(u_p,0)=tmpf0*cos(yf0)*cos(zf0);
                // node(u_p,1)=tmpf1*cos(xf1)*cos(zf1);
                // node(u_p,2)=tmpf2*cos(xf2)*cos(yf2);
                // node(nl_jac_exact,0)=tmpf0*tmpf0*(
                //     2*a_*cos(xf0)*(yf0*(2*xf0*xf0-zf0*zf0)*cos(yf0)+(2*xf0*xf0-yf0*yf0)*zf0*cos(zf0))+
                //     xf0*(zf0*cos(zf0)*(sin(xf0)-sin(yf0))+yf0*cos(yf0)*(-4*a_*zf0*cos(zf0)+sin(xf0)-sin(zf0)))
                // );
                // node(nl_jac_exact,1)=tmpf1*tmpf1*(
                //     zf1*cos(zf1)*(-2*a_*(xf1*xf1-2*yf1*yf1)*cos(yf1)+yf1*(-sin(xf1)+sin(yf1)))+
                //     xf1*cos(xf1)*(2*a_*(2*yf1*yf1-zf1*zf1)*cos(yf1)+yf1*(-4*a_*zf1*cos(zf1)+sin(yf1)-sin(zf1)))
                // );

                // node(nl_jac_exact,2)=tmpf2*tmpf2*(
                //     xf2*cos(xf2)*(-2*a_*(yf2*yf2-2*zf2*zf2)*cos(zf2)-zf2*(4*a_*yf2*cos(yf2)+sin(yf2))+zf2*sin(zf2))+
                //     yf2*cos(yf2)*(-2*a_*(xf2*xf2-2*zf2*zf2)*cos(zf2)+zf2*(-sin(xf2)+sin(zf2)))

                // );

                // node(nl_jac_T_exact, 0) = tmpf0*tmpf0 * (yf0 * (1 - 2 * a_ * (xf0 * xf0 + zf0 * zf0)) * cos(xf0) * cos(yf0) +
                //                                      zf0 * cos(zf0) *
                //                                          (cos(xf0) - 2 * a_ * (xf0 * xf0 + yf0 * yf0) * cos(xf0) +
                //                                              xf0 * (8 * a_ * yf0 * cos(yf0) + sin(yf0))) +
                //                                      xf0 * yf0 * cos(yf0) * sin(zf0));
                // node(nl_jac_T_exact, 1) =
                //     tmpf1 *tmpf1* (zf1 * cos(zf1) * (cos(yf1) - 2 * a_ * (xf1 * xf1 + yf1 * yf1) * cos(yf1) + yf1 * sin(xf1)) +
                //                xf1 * cos(xf1) *
                //                    (cos(yf1) - 2 * a_ * (yf1 * yf1 + zf1 * zf1) * cos(yf1) +
                //                        yf1 * (8 * a_ * zf1 * cos(zf1) + sin(zf1))));

                // node(nl_jac_T_exact, 2) =
                //     tmpf2 * tmpf2* (yf2 * cos(yf2) * (cos(zf2) - 2 * a_ * (xf2 * xf2 + zf2 * zf2) * cos(zf2) + zf2 * sin(xf2)) +
                //                xf2 * cos(xf2) *
                //                    (8 * a_ * yf2 * zf2 * cos(yf2) + cos(zf2) - 2 * a_ * (yf2 * yf2 + zf2 * zf2) * cos(zf2) +
                //                        zf2 * sin(yf2)));
                ...
                node(u_p, 1) = tmpf1;
                node(u_p, 2) = tmpf2;
                node(nl_jac_exact, 0) = 4*a_*tmpf0*tmpf0*(2*xf0-yf0-zf0);
                node(nl_jac_exact, 1) = -4*a_*tmpf1*tmpf1*(xf1-2*xf1+zf1);
                node(nl_jac_exact, 2) = -4*a_*tmpf2*tmpf2*(xf2+yf2-2*zf2);
                node(nl_jac_T_exact, 0) = 2*a_*tmpf0*tmpf0*(-2*xf0+yf0+zf0);
                node(nl_jac_T_exact, 1) = 2*a_*tmpf1*tmpf1*(xf1-2*yf1+zf1);
                node(nl_jac_T_exact, 2) = 2*a_*tmpf2*tmpf2*(xf2+yf2-2*zf2);
