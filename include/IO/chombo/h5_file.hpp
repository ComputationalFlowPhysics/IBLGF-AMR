...
//Basic hdf5-types
template<typename T> struct hdf_type { static hid_t type(){throw std::runtime_error("no hdf5_type registered");return 0;}};
template<typename T> struct hdf_type<std::vector<T>> { static hid_t type(){return hdf_type<T>::type();}};
template<> struct hdf_type<double>     { static hid_t type(){return H5T_NATIVE_DOUBLE;}};
template<> struct hdf_type<float>      { static hid_t type(){return H5T_NATIVE_FLOAT;}};
template<> struct hdf_type<char>       { static hid_t type(){return H5T_FORTRAN_S1;}};
template<> struct hdf_type<const char> { static hid_t type(){return H5T_NATIVE_INT;}};
template<> struct hdf_type<int>        { static hid_t type(){return H5T_NATIVE_UINT;}};
#define HDF5_CHECK_ERROR(ID, MSG) if (ID<0) throw std::runtime_error(MSG);
    ...
    public:
       ...
       //     H5Pclose(dtplist_id);
        ...
        hid_type create_group(hid_type loc_id,std::string _group_identifier)
        {
            ...
            if (!non_existent){
                auto group_id=H5Gcreate(loc_id,  _group_identifier.c_str(), H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
                HDF5_CHECK_ERROR(group_id,"HDF5-Error: Could not create group: "+ _group_identifier ) ;
                return group_id;
            }
            else{
                //update_plist();
        ...
        hid_type create_simple_space(int rank=1, const hsize_t* dims=-1, const hsize_t * max_dims=NULL)
        {
        ...
        template<typename T >
        hid_type create_dataset(hid_type location_id, std::string _dset_name, hid_type dataspace_id )
        {
            ...
            auto dataset_id=H5Dcreate(location_id, _dset_name.c_str(), hdf_t, dataspace_id, H5P_DEFAULT, dcplist_id, H5P_DEFAULT);
            ...
            HDF5_CHECK_ERROR(dataset_id,"HDF5-Error: Could not create dset: "+ _dset_name ) ;
            H5Pclose(dcplist_id);
            H5Pclose(dtplist_id);
            return dataset_id;
        }
        ...
        std::array<hsize_type,dimension> dataspace_dimensions(const hid_type& _dset_id)
        {
            std::array<hsize_type,dimension> dims;
            const int dim_check=H5Sget_simple_extent_dims(get_dataspace(_dset_id), &dims[0], NULL);
            HDF5_CHECK_ERROR(dim_check,"HDF5-Error: Could not fetch space dimensions");
      ...
      //  void write(hid_type& dset_id, hsize_type dimsf, auto buf)
        {
            ...
            H5Sselect_hyperslab(filespace, H5S_SELECT_SET,
                            &offset, NULL, &block_data_size, NULL);
        ...
        template<typename T>
        void read_dataset(hid_t& dataset_id, hid_t& memspace_id,hid_t& dataspace_id, T* data )
        {
            auto hdf_t= hdf_type<T>::type();
            auto status= H5Dread(dataset_id, hdf_t, memspace_id, dataspace_id, H5P_DEFAULT, data);
        ...
        std::vector<std::size_t> read_dataset_dimsion(hid_t dataset_id )
        {
            ...
            for(unsigned int d=0;d<dimension;++d) res[d]=static_cast<std::size_t>(dims[d]);
            return res;
        }
        ...
        template<typename T>
        void write_hyperslab_cont1D(hid_type& dset_id, hyperslab<1> _hyperslab, T* buf)
        {
            auto filespace = H5Dget_space(dset_id);
            auto err= H5Sselect_hyperslab(filespace, H5S_SELECT_SET, &_hyperslab.offset_[0],  NULL, &_hyperslab.count_[0], NULL);
...
//            H5Pset_dxpl_mpio(plist_id, H5FD_MPIO_INDEPENDENT);
        ...
        template<typename T, std::size_t Dset_Dim=NumDims>
        void write_hyperslab(hid_type& dset_id, hyperslab<Dset_Dim> _hyperslab, T* buf)
        {
            auto filespace = H5Dget_space(dset_id);
            auto err= H5Sselect_hyperslab(filespace, H5S_SELECT_SET, &_hyperslab.offset_[0],  &_hyperslab.stride_[0], &_hyperslab.count_[0], &_hyperslab.block_[0]);
            HDF5_CHECK_ERROR(err,"HDF5-Error: Could not select hyperslab ") ;

            auto memspace_id = H5Screate_simple(Dset_Dim, &_hyperslab.chunk_dims_[0], NULL);
 ...
 //           H5Pset_dxpl_mpio(plist_id, H5FD_MPIO_COLLECTIVE);
        ...
        {
            inline static void create(hdf5_file* h5, hid_type& _group_id,std::string _attr_identifier, const  T& _attr_value)
            {
                hsize_type dims_at     = 1;
                hid_type dataspace_at  = H5Screate_simple(1, &dims_at, NULL);

                auto hdf_t= hdf_type<T>::type();
                hid_t att_id = H5Acreate(_group_id, _attr_identifier.c_str(), hdf_t , dataspace_at, H5P_DEFAULT, H5P_DEFAULT);
                auto ret = H5Awrite(att_id, hdf_t, &_attr_value);
                HDF5_CHECK_ERROR(ret,"HDF5-Error: Could not write attr: " + _attr_identifier );
                h5->close_space(dataspace_at);
                h5->close_attribute(att_id);
            }

            inline static T read( hdf5_file* h5,hid_type& _group_id  ,std::string _attr_identifier )
            {
                T res{};
        ...
        {
            inline static void create(hdf5_file* h5,hid_type& _group_id,std::string _attr_identifier,  std::vector<T> _attr_value)
            {
                ...
                HDF5_CHECK_ERROR(att_id,"HDF5-Error: Could not write attr: " + _attr_identifier );
                h5->close_space(dataspace_at);
                h5->close_attribute(att_id);
            }

            inline static std::vector<T> read(hdf5_file* h5,hid_type& _group_id,std::string _attr_identifier)
            {
                std::vector<T> res;
                hsize_t dims;
                auto hdf_t= hdf_type<std::vector<T>>::type();
        ...
        {
            //Write string attr:
            inline static void create(hdf5_file* h5,hid_type& _group_id,std::string _attr_identifier,  std::string _attr_value)
            {
                ...
                auto attr_id = H5Acreate2(_group_id , _attr_identifier.c_str(), atype, aid3, H5P_DEFAULT, H5P_DEFAULT);
                auto ret = H5Awrite(attr_id, atype, string_ptr);
                HDF5_CHECK_ERROR(ret,"HDF5-Error: Could not write string attr: " + _attr_value );
                h5->close_space(aid3);
            ...
            inline static std::string read( hdf5_file* h5,hid_type& _group_id ,std::string _attr_identifier )
            {
                auto attr = H5Aopen (_group_id,  _attr_identifier.c_str(), H5P_DEFAULT);
                auto filetype = H5Aget_type (attr);
                size_t sdim = H5Tget_size (filetype);
        ...
        //interface
        template<class T, class U=void>
        inline void create_attribute(hid_type& _group_id,std::string _attr_identifier,  T _attr_value)
        {
            attribute_helper<T, U>::create(this,_group_id,_attr_identifier,  _attr_value);
        }
        ...
        void write_boxCompound(hid_type& _group_id, std::string _cName, std::vector<index_list_t>& _min, std::vector<index_list_t>& _max, bool asAttr=true)
        {
        ...
        template<std::size_t ND =dimension>
        void write_boxCompound(hid_type& _group_id,std::string _cName, index_list_t& _min, index_list_t& _max, bool asAttr=true)
        {
        ...
        // CREATE + WRITE *******************************************************
        // for 2D
        void write_boxCompound(hid_type& _group_id,std::string _cName, box_compound* _c, std::integral_constant<std::size_t, 2>* ,hsize_type _dim=1,bool asAttr=true)
        {
            ...
            H5Tinsert(s1_tid, _c->lo_i_str.c_str(), HOFFSET(box_compound, lo_i), H5T_NATIVE_INT);
            H5Tinsert(s1_tid, _c->lo_j_str.c_str(), HOFFSET(box_compound, lo_j), H5T_NATIVE_INT);
            H5Tinsert(s1_tid, _c->hi_i_str.c_str(), HOFFSET(box_compound, hi_i), H5T_NATIVE_INT);
            H5Tinsert(s1_tid, _c->hi_j_str.c_str(), HOFFSET(box_compound, hi_j), H5T_NATIVE_INT);
                                ...
                                hsize_type _dim=1,bool asAttr=true)
        {
            ...
            H5Tinsert(s1_tid, _c->lo_i_str.c_str(), HOFFSET(box_compound, lo_i), H5T_NATIVE_INT);
            H5Tinsert(s1_tid, _c->lo_j_str.c_str(), HOFFSET(box_compound, lo_j), H5T_NATIVE_INT);
            H5Tinsert(s1_tid, _c->lo_k_str.c_str(), HOFFSET(box_compound, lo_k), H5T_NATIVE_INT);
            H5Tinsert(s1_tid, _c->hi_i_str.c_str(), HOFFSET(box_compound, hi_i), H5T_NATIVE_INT);
            H5Tinsert(s1_tid, _c->hi_j_str.c_str(), HOFFSET(box_compound, hi_j), H5T_NATIVE_INT);
            H5Tinsert(s1_tid, _c->hi_k_str.c_str(), HOFFSET(box_compound, hi_k), H5T_NATIVE_INT);
        ...
        // for 3D
        void create_boxCompound( hid_type& _group_id, std::string _cName,box_compound* _c,std::integral_constant<std::size_t, 3>*,hsize_type _dim=1,bool asAttr=true)
        {
            //create memory for the dataType:
            const hsize_type  rank=1;
            const hsize_type  dim=_dim;
        ...
        //        auto status = H5Awrite(dataset, s1_tid, _c);
            ...
            {
                auto dataset = H5Dcreate(_group_id, _cName.c_str(),
                        s1_tid, space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
        //        auto status = H5Dwrite(dataset, s1_tid, H5S_ALL, H5S_ALL, H5P_DEFAULT, _c);
                close_dset(dataset);
            }
            close_space(space);
        ...
        // write vector of boxes (boxes, patch_offsets)
        template<std::size_t ND =dimension>
        void open_write_boxCompound(hid_type& _group_id, std::string _cName, std::vector<index_list_t>& _min, std::vector<index_list_t>& _max, bool asAttr=true)
        {
            std::vector<box_compound> boxes;
            for(unsigned int i=0;i<_min.size();++i)
            {
            ...
            using tag=std::integral_constant<std::size_t,ND>*;
            hsize_type size=static_cast<hsize_type>(_min.size());
            open_write_boxCompound(_group_id,_cName, &boxes[0], tag(0),size,asAttr);
        }
        ...
        // write one box (probDomain)
        template<std::size_t ND =dimension>
        void open_write_boxCompound(hid_type& _group_id,std::string _cName, index_list_t& _min, index_list_t& _max, bool asAttr=true)
        {
            box_compound _c(_min, _max);
            using tag=std::integral_constant<std::size_t,ND>*;
        ...
        // for 2D
        void open_write_boxCompound(hid_type& _group_id,std::string _cName, box_compound* _c, std::integral_constant<std::size_t, 2>* ,hsize_type _dim=1,bool asAttr=true)
        {
            auto s1_tid = H5Tcreate (H5T_COMPOUND, sizeof(*_c));
            H5Tinsert(s1_tid, _c->lo_i_str.c_str(), HOFFSET(box_compound, lo_i), H5T_NATIVE_INT);
            H5Tinsert(s1_tid, _c->lo_j_str.c_str(), HOFFSET(box_compound, lo_j), H5T_NATIVE_INT);
        ...
        // for 3D
        void open_write_boxCompound( hid_type& _group_id, std::string _cName,box_compound* _c,std::integral_constant<std::size_t, 3>*,hsize_type _dim=1,bool asAttr=true)
        {
            auto s1_tid = H5Tcreate (H5T_COMPOUND, sizeof(*_c));
            H5Tinsert(s1_tid, _c->lo_i_str.c_str(), HOFFSET(box_compound, lo_i), H5T_NATIVE_INT);
            H5Tinsert(s1_tid, _c->lo_j_str.c_str(), HOFFSET(box_compound, lo_j), H5T_NATIVE_INT);
        ...
        //Some info functions
        std::vector<std::pair<std::string, hid_type> > get_all_datasets(hid_type _g_id)
        {
            ...
            HDF5_CHECK_ERROR(err,"HDF5-Error: Could get number of objects in group");
        ...
        void scan_attr(hid_type _attr_id) noexcept
        {
	        ...
	        auto aspace = H5Aget_space(_attr_id); /* the dimensions of the attribute data */
        ...
        void scan_dataset(hid_type _dset_id)
        {
	        ...
	        sid = H5Dget_space(_dset_id); /* the dimensions of the dataset (not shown) */
        ...
        void scan_plist(hid_type _pid) noexcept
        {
	        ...
	        for (i = 0; i < nfilters; i++)
	        {
	        	 ...
	        	 */
	        	cd_nelmts = 32;
	        	filtn = H5Pget_filter(_pid, (unsigned)i,
	        		&filt_flags, &cd_nelmts, cd_values,
	        		(size_t)maxChar, f_name, &filt_conf);
  	        	/*
