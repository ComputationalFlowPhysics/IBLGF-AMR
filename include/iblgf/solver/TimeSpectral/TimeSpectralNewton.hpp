...
#ifndef IBLGF_INCLUDED_TIME_SPECTRAL_HPP
#define IBLGF_INCLUDED_TIME_SPECTRAL_HPP
...
class TimeSpectral
{
	...
	TimeSpectral(simulation_type* _simulation, int _N_fourier_modes)
    ...
    , ifherk(simulation_) 
	{
        ...
        //N_fourier_modes = simulation_->dictionary_->template get_or<float_type>("N_fourier_modes",5);
        N_fourier_modes = _N_fourier_modes;

        pcout << "Number of Fourier modes is: " << N_fourier_modes << std::endl;

        x_vec.resize(N_fourier_modes * 2);
        x_real_vec.resize(N_fourier_modes * 3);
        x_comp_vec.resize(N_fourier_modes * 2);
        x_old_vec.resize(N_fourier_modes * 2);
        b_vec.resize(N_fourier_modes * 2);

        b_real_vec.resize(N_fourier_modes * 3);
        b_comp_vec.resize(N_fourier_modes * 2);
        ...
        if (world.rank() != 0) {
            ...
            r2cFunc_vec.emplace_back(std::make_unique<fft::helm_dfft_r2c>(N_fourier_modes*3, N_fourier_modes*2,
                    loc_size, 1));

            c2rFunc_vec.emplace_back(std::make_unique<fft::helm_dfft_c2r>(N_fourier_modes*3, N_fourier_modes*2,
                    loc_size, 1));
            ...
            for (int i = 0; i < N_fourier_modes * 2;i++) {
                x_vec[i]         = (float_type*)MKL_malloc(sizeof(float_type) * loc_size, 64);
                x_old_vec[i]     = (float_type*)MKL_malloc(sizeof(float_type) * loc_size, 64);
                x_comp_vec[i]    = (float_type*)MKL_malloc(sizeof(float_type) * loc_size, 64);
                b_vec[i]         = (float_type*)MKL_malloc(sizeof(float_type) * loc_size, 64);

                b_comp_vec[i]         = (float_type*)MKL_malloc(sizeof(float_type) * loc_size, 64);
            ...
            for (int i = 0; i < N_fourier_modes * 3;i++) {
                x_real_vec[i]    = (float_type*)MKL_malloc(sizeof(float_type) * loc_size, 64);
                b_real_vec[i]    = (float_type*)MKL_malloc(sizeof(float_type) * loc_size, 64);
    ...
    float_type NewtonRHS_vec(int idx, force_type& forcing_vec, float_type& res_inf, float_type& state_err) {
        ...
        ifherk.template CSR2Grid<Face, Cell, Edge>(x[idx], forcing_vec);

        real_coordinate_type tmp_coord(0.0);
        forcing_tmp.resize(domain_->ib().size());

        ifherk.template NewtonRHS_No_nonlinear<Face, Cell, fu_i_type, fp_i_type>(forcing_vec, forcing_tmp);
        ...
        if (world.rank() != 0) {
            ...
            std::vector<float_type> v(3*N_fourier_modes*loc_size, 0.0);

            std::vector<std::complex<float_type>> x_comp(N_fourier_modes*loc_size, 0.0);

            for (int i = 0; i < N_fourier_modes; i++) {
                for (int j = 0; j < loc_size; j++) {
                    long long idx = j * N_fourier_modes + i;
                    //v[idx] = x_vec[i][j];
                    std::complex<float_type> tmp_val(x_vec[2*i][j],x_vec[2*i+1][j]);
                    x_comp[idx] = tmp_val;
                    
                }
            }

            c2rFunc_vec[0].copy_field(x_comp);
            c2rFunc_vec[0].execute();
            std::vector<float_type> output_vel;
            c2rFunc_vec[0].output_field(output_vel);

            for (int i = 0; i < N_fourier_modes*3; i++) {
                for (int j = 0; j < loc_size; j++) {
                    long long idx = j * N_fourier_modes * 3 + i;
                    //v[idx] = x_vec[i][j];
                    x_real_vec[i][j] = output_vel[idx]; 
                }
            }
            ...
            for (int i = 0; i < N_fourier_modes*3; i++) {
                ...
                ifherk.template CSR2Grid<Face, Cell, Edge>(x_real_vec[idx], forcing_vec);
                ifherk.clean<fp_i_type>();
                real_coordinate_type tmp_coord(0.0);
                force_type forcing_tmp;
                forcing_tmp.resize(domain_->ib().size());
                ifherk.template NewtonRHS_nonlinear<Face, Face, fu_i_type>();
            ...
            for (int i = 0; i < N_fourier_modes*3; i++) {
                for (int j = 0; j < loc_size; j++) {
                    long long idx = j * N_fourier_modes * 3 + i;
                    //v[idx] = x_vec[i][j];
                    v[idx] = b_real_vec[i][j];
                }
            }

            r2cFunc_vec[0].copy_field(v);
            r2cFunc_vec[0].execute();
            std::vector<std::complex<float_type>> output_comp;
            r2cFunc_vec[0].output_field(output_comp);
            ...
            for (int i = 0; i < N_fourier_modes; i++) {
                for (int j = 0; j < loc_size; j++) {
                    long long idx = j * N_fourier_modes + i;
                    //v[idx] = x_vec[i][j];
                    b_comp_vec[2*i][j] = output_comp[idx].real() / static_cast<float_type>(N_fourier_modes*3);
                    b_comp_vec[2*i+j][j] = output_comp[idx].imag() / static_cast<float_type>(N_fourier_modes*3);                    
                }
            }
            ...
            ifherk.template Grid2CSR<fu_i_type, fp_i_type, fw_i_type>(b_vec[idx], forcing_tmp);

        }

        for (int i = 0; i < N_fourier_modes) {
    ...
    std::vector<std::unique_ptr<fft::helm_dfft_c2r>> c2rFunc_vec;

    std::vector<float_type*> x_vec, x_old_vec, b_vec, x_real_vec, x_comp_vec, b_real_vec, b_comp_vec;
