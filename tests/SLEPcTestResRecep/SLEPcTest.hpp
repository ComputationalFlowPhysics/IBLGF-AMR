...
Mat Rep_Forcing_mat, Recep_DN_forcing;
...
PetscErrorCode MatMul_ATA_inv(Mat A_, Vec x_, Vec y_) {
    ...
    PetscCall(MatShellGetContext(A_,&ctx));
    nx = *(int*)ctx;
    for (PetscInt i = rstart; i < rend; i++) {
        PetscScalar tmp;
        int nextAdd = i+ CurDim + rstart;
        int cur_idx = i + rstart;
        PetscCall(VecGetValues(x_, 1, &cur_idx, &tmp));
        PetscCall(VecSetValues(By, 1, &i, &tmp, INSERT_VALUES));
        PetscCall(VecGetValues(x_, 1, &nextAdd, &tmp));
        PetscCall(VecSetValues(Cy, 1, &i, &tmp, INSERT_VALUES));
    }
    PetscCall(VecAssemblyBegin(Cy));
    PetscCall(VecAssemblyEnd(Cy));
    ...
    for (PetscInt i = rstart; i < rend; i++) {
        ...
        int nextAdd = i+ CurDim + rstart;
        int cur_idx = i + rstart;
        PetscCall(VecGetValues(Cx1, 1, &i, &tmp));
        PetscCall(VecSetValues(y_, 1, &cur_idx, &tmp, INSERT_VALUES));
        PetscCall(VecGetValues(Cx2, 1, &i, &tmp));
        PetscCall(VecSetValues(y_, 1, &nextAdd, &tmp, INSERT_VALUES));
    }
    PetscCall(VecAssemblyBegin(y_));
    PetscCall(VecAssemblyEnd(y_));
    PetscFunctionReturn(0);
}
...
PetscErrorCode MatMul_ATA_inv_cross(Mat A_, Vec x_, Vec y_) {
    PetscCall(MatMult(Rep_Forcing_mat, x_, Cy));
    PetscCall(MatMult(Recep_DN_forcing, Cy, Cx1));

    //PetscCall(MatMult(Q_hi, Cx1, By));
    PetscCall(KSPSolve(kspA, Cx1, y_tmp));
    PetscCall(MatMult(C, y_tmp, By));
    PetscCall(MatMult(C, By, y_tmp));
    PetscCall(KSPSolve(kspAT, y_tmp, Cy));
    //PetscCall(MatMult(Q_hi, Cy, By));

    PetscCall(MatMultHermitianTranspose(Recep_DN_forcing, Cy, Cx1));
    PetscCall(MatMultHermitianTranspose(Rep_Forcing_mat, Cx1, y_));

    PetscFunctionReturn(0);
}

/*PetscErrorCode MatMul_ForcingInVel(Mat A_, Vec x_, Vec y_) {
    //Get the forcing in velocity
    PetscCall(MatMult(Rep_Forcing_mat, x_, y_));
    PetscFunctionReturn(0);
}*/

PetscErrorCode MatMul_ATA_inv_cross_Res(Mat A_, Vec x_, Vec y_) {
    //PetscCall(MatMult(Rep_Forcing_mat, x_, Cy));
    //PetscCall(MatMult(Recep_DN_forcing, Cy, Cx1));
    ...
    //PetscCall(MatMultHermitianTranspose(Recep_DN_forcing, By, Cx1));
    //PetscCall(MatMultHermitianTranspose(Rep_Forcing_mat, Cx1, y_));

    PetscFunctionReturn(0);
}

/*PetscErrorCode MatMulForcing(Mat A_, Vec x_, Vec y_) {
    PetscCall(MatShellGetContext(A_,&ctx));
    nx = *(int*)ctx;
    for (PetscInt i = rstart; i < rend; i++) {
        PetscScalar tmp;
    ...
    for (PetscInt i = rstartF; i < rendF; i++) {
        PetscScalar tmp;
        int nextAdd = i+ CurDimF + rstart;
        //int cur_idx = i + rstart;
        //PetscCall(VecGetValues(Cx1, 1, &i, &tmp));
        //PetscCall(VecSetValues(y_, 1, &cur_idx, &tmp, INSERT_VALUES));
        PetscCall(VecGetValues(x_, 1, &nextAdd, &tmp));
    ...
    PetscCall(MatMult(Rep_Forcing_mat, Cy, Cx1));
    PetscCall(MatMult(C, By, y_));
    PetscFunctionReturn(0);
}*/
    ...
    Dim,
     (
        ...
        //IF-HERK
         ...
         (vort_idicator    , float_type, 1,    1,       1,     cell,true  )
...
struct NS_AMR_LGF : public SetupHelmStab<NS_AMR_LGF, parameters>
{
			...
			})
	{
        ...
        forcingLx = simulation_.dictionary_->template get_or<float_type>("forcingLx", 100);

        max_vort_forcing = simulation_.dictionary_->template get_or<float_type>("vort_mag_forcing", 1e-5);
	...
	float_type run(int argc, char *argv[])
	{
        ...
        ifherk.constructing_forcing_mat(Omega_l);
        //ifherk.constructing_forcing_mat_diff_domain(Omega_l, forcingLx);
        
        //ifherk.construction_DN_forcing<u_type, uz_type>();

        ifherk.construction_DN_forcing_vor_level<u_type, uz_type, vort_idicator_type>(max_vort_forcing);
        //ifherk.constructing_forcing_mat_vor_level<edge_aux_type, vort_idicator_type>(Omega_l, max_vort_forcing);
        ifherk.add_IB_forcing_mat();
        ...
        //Vec*           Cv;               /*deflation of 
        Vec            b_res, f_res;       //result of B_res, having the dimension of velocity only
        Mat            A,B, A_shell, AT;             /* linear system matrix */
        Mat            Rep_Forcing_mat_real, Rep_Forcing_mat_imag;
        ...
        PetscCall(VecDuplicate(x, &Cx1));
        PetscCall(VecDuplicate(x, &Cx2));
        ...
        PetscCall(VecSetSizes(u, loc_size*2, n*2));
        ...
        for (i = rstart; i < rend; i++)
        {
            ...
            for (const auto& [key, val] : rowQ)
            {
                ...
                PetscScalar valComplex = std::sqrt(val); //sqrt for the consideration of the inner product
        ...
        //initialize the receptivity matrix

        int nlocal_forcing = ifherk.loc_ib_pts;
        int n_forcing = ifherk.forcing_coords.size();

        PetscCall(VecCreate(PETSC_COMM_WORLD, &f_res));
        PetscCall(VecSetSizes(f_res, nlocal_forcing, n_forcing));
        PetscCall(VecSetFromOptions(f_res));
        PetscCall(VecGetOwnershipRange(f_res, &rstartF, &rendF));

        PetscCall(MatCreate(PETSC_COMM_WORLD, &Rep_Forcing_mat));
        MatSetType(Rep_Forcing_mat,MATMPIDENSE);
        //PetscCall(MatCreateMPIAIJWithArrays(PETSC_COMM_WORLD, nlocal, nlocal, n, n, ia, ja, a, &A));
        PetscCall(MatSetSizes(Rep_Forcing_mat, nlocal, nlocal_forcing, n, n_forcing));
        PetscCall(MatSetFromOptions(Rep_Forcing_mat));
        PetscCall(MatSetUp(Rep_Forcing_mat));

        PetscCall(MatCreate(PETSC_COMM_WORLD, &Rep_Forcing_mat_imag));
        MatSetType(Rep_Forcing_mat_imag,MATMPIDENSE);
        //PetscCall(MatCreateMPIAIJWithArrays(PETSC_COMM_WORLD, nlocal, nlocal, n, n, ia, ja, a, &A));
        PetscCall(MatSetSizes(Rep_Forcing_mat_imag, nlocal, nlocal_forcing, n, n_forcing));
        PetscCall(MatSetFromOptions(Rep_Forcing_mat_imag));
        PetscCall(MatSetUp(Rep_Forcing_mat_imag));

        PetscCall(MatCreate(PETSC_COMM_WORLD, &Recep_DN_forcing));
        MatSetType(Recep_DN_forcing,MATMPIAIJ);
        //PetscCall(MatCreateMPIAIJWithArrays(PETSC_COMM_WORLD, nlocal, nlocal, n, n, ia, ja, a, &A));
        PetscCall(MatSetSizes(Recep_DN_forcing, nlocal, nlocal, n, n));
        PetscCall(MatSetFromOptions(Recep_DN_forcing));
        PetscCall(MatSetUp(Recep_DN_forcing));

        load_PETSc_matrix(ifherk.ForcingMat_real, Rep_Forcing_mat, rstart, rend, true);
        load_PETSc_matrix(ifherk.ForcingMat_imag, Rep_Forcing_mat_imag, rstart, rend, false);
        load_PETSc_matrix(ifherk.DN_forcing, Recep_DN_forcing, rstart, rend, true);

        MatAssemblyBegin(Rep_Forcing_mat,MAT_FINAL_ASSEMBLY);
        MatAssemblyEnd(Rep_Forcing_mat,MAT_FINAL_ASSEMBLY);

        MatAssemblyBegin(Rep_Forcing_mat_imag,MAT_FINAL_ASSEMBLY);
        MatAssemblyEnd(Rep_Forcing_mat_imag,MAT_FINAL_ASSEMBLY);

        MatAXPY(Rep_Forcing_mat, 1, Rep_Forcing_mat_imag, UNKNOWN_NONZERO_PATTERN);
        MatAssemblyBegin(Rep_Forcing_mat,MAT_FINAL_ASSEMBLY);
        MatAssemblyEnd(Rep_Forcing_mat,MAT_FINAL_ASSEMBLY);

        MatAssemblyBegin(Recep_DN_forcing,MAT_FINAL_ASSEMBLY);
        MatAssemblyEnd(Recep_DN_forcing,MAT_FINAL_ASSEMBLY);
        ...
        //PetscCall(MatCreateShell(PETSC_COMM_WORLD,2*nlocal, 2*nlocal, 2*n, 2*n,&ctx,&A_shell));
        //PetscCall(MatShellSetOperation(A_shell,MATOP_MULT,(void(*)(void))MatMul_ATA_inv));
        //For forcing
        PetscCall(MatCreateShell(PETSC_COMM_WORLD,nlocal_forcing, nlocal_forcing, n_forcing, n_forcing,&ctx,&A_shell));
        PetscCall(MatShellSetOperation(A_shell,MATOP_MULT,(void(*)(void))MatMul_ATA_inv_cross));

        //for resolvent
        //PetscCall(MatCreateShell(PETSC_COMM_WORLD,nlocal, nlocal, n, n,&ctx,&A_shell));
        //PetscCall(MatShellSetOperation(A_shell,MATOP_MULT,(void(*)(void))MatMul_ATA_inv_cross_Res));
  ...
  */
        ...
        //EPSSetProblemType(eps,EPS_NHEP); //should be HEP, but NHEP is set for debugging purpose
  ...
  */
     ...
     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
        ...
        for (float_type Cur_o = Omega_int+Omega_l; Cur_o <= Omega_r; Cur_o+= Omega_int) 
        {
            ...
            ifherk.update_forcing_mat(Cur_o);
            ...
            outfile << std::setw(width) << std::sqrt(evr) << std::setw(width)
                    << std::scientific << std::setprecision(9);
            outfile << std::endl;
            outfile.close();
            }
        ...
        EPSGetEigenvector(eps, 0, f_res, NULL);

        forcingF.resize(n_forcing);
        forcingF_glob.resize(n_forcing);
        std::fill(forcingF.begin(), forcingF.end(), 0.0);
        std::fill(forcingF_glob.begin(), forcingF_glob.end(), 0.0);

        for (i = rstartF; i < rendF; i++) {
            PetscScalar tmp;
            PetscCall(VecGetValues(f_res, 1, &i, &tmp));
            int idx = i;
            forcingF[idx] = PetscRealPart(tmp);
        }

        boost::mpi::all_reduce(domain_->client_communicator(), forcingF.data(), n_forcing, forcingF_glob.data(), std::plus<double>());

        //MPI_Allreduce(forcingF.data(), forcingF_glob.data(), n_forcing, float_type, MPI_SUM, PETSC_COMM_WORLD);

        if (world.rank() == 1) {

            std::ofstream myfile;
            myfile.open("Forcing.txt", std::ios_base::app);

            for (int i = 0; i < n_forcing; i++) {
                myfile << ifherk.forcing_coords[i].x() << " " 
                       << ifherk.forcing_coords[i].y() << " " 
                       << forcingF_glob[i] << std::endl;
            }
            myfile.close();
        ...
        PetscCall(MatMult(Rep_Forcing_mat, f_res, y));
        ...
        /*EPSGetEigenvector(eps,0, u, NULL);

        for (i = rstart; i < rend; i++) {
            PetscScalar tmp;
            int nextAdd = i + loc_size + rstart;
            int cur_idx = i + rstart;
            PetscCall(VecGetValues(u, 1, &nextAdd, &tmp));
            PetscCall(VecSetValues(x, 1, &i, &tmp, INSERT_VALUES));
            PetscCall(VecGetValues(u, 1, &cur_idx, &tmp));
            PetscCall(VecSetValues(y_tmp, 1, &i, &tmp, INSERT_VALUES));
        }
        PetscCall(VecAssemblyBegin(x));
        PetscCall(VecAssemblyEnd(x));
        PetscCall(VecAssemblyBegin(y_tmp));
        ...
        simulation_.writeWithCorr("x3_imag_corr.hdf5");
    ...
    void load_PETSc_matrix(sparse_mat& matrix, Mat& PETSc_mat_, PetscInt rstart, PetscInt rend, bool real) {
        PetscInt i_;
        for (i_ = rstart; i_ < rend; i_++)
        {
            //int range = rend - rstart;
            //int range_nonzero = range/2;
            int i_loc = i_ - rstart;
            ...
            std::map<int, float_type> row = matrix.mat[i_loc+1];
            ...
            MatSetValues(PETSc_mat_, 1, &i_, counter, row_idx.data(), row_val.data(), INSERT_VALUES);
        }
    }
    ...
    std::vector<float_type> forcingF, forcingF_glob; //this is the forcing vector for recpticity problem
    ...
    float_type forcingLx; //Lx for defining the forcing region
    float_type max_vort_forcing; //max forcing region according to vorticity
