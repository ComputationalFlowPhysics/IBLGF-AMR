...
#ifndef IBLGF_INCLUDED_SETUP_BASE_HELM_OMP_HPP
#define IBLGF_INCLUDED_SETUP_BASE_HELM_OMP_HPP
...
#include <iblgf/solver/time_integration/ifherk_helm_OMP.hpp>
...
class Setup_helmholtz_OMP
: private crtp::Crtps<Setup, Setup_helmholtz_OMP<Setup, SetupTraits>>
, public SetupTraits
{
  ...
  public: //default fields
    ...
    (Dim,
    (
      (source_tmp,          float_type,  1*2*N_modes,  1,  1,  cell,false),
      (correction_tmp,      float_type,  1*2*N_modes,  1,  1,  cell,false),
      (target_tmp,          float_type,  1*2*N_modes,  1,  1,  cell,false),
      (fmm_s,               float_type,  1*2*N_modes,  1,  1,  cell,false),
      (fmm_t,               float_type,  1*2*N_modes,  1,  1,  cell,false),
      //flow variables
  ...
  public: //Trait types to be used by others
    ...
    using Fmm_t = Fmm_OMP<Setup_helmholtz_OMP>;
    using fmm_mask_builder_t = FmmMaskBuilder<domain_t>;
    using poisson_solver_t = solver::PoissonSolverOMP<Setup_helmholtz_OMP>;
    using time_integration_t = solver::Ifherk_HELM_OMP<Setup_helmholtz_OMP>;
    using linsys_solver_t = solver::LinSysSolver_helm_OMP<Setup_helmholtz_OMP>;

  public: //Ctors
    Setup_helmholtz_OMP(Dictionary* _d)
    ...
    Setup_helmholtz_OMP(Dictionary* _d, domaint_init_f _fct,
