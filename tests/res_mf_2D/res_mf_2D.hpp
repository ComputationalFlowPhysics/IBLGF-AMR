...
#include <iblgf/solver/matrix_free/rsvd_dt.hpp>
namespace iblgf
{
const int Dim = 2;
...
struct NS_AMR_LGF : public SetupLinear<NS_AMR_LGF, parameters>
{
    ...
    float_type run(int argc, char* argv[])
    {
        ...
        solver::RSVD_DT<SetupLinear> rsvd_dt(&this->simulation_);
        rsvd_dt.run();
    ...
    void initialize()
    {
        ...
        for (auto it = domain_->begin(); it != domain_->end(); ++it)
        {
            ...
            for (auto& node : it->data())
            {
                ...
                node(u_base, 0) =  u_vort(x, y, 0, 0);
                node(u_base, 1) =  u_vort(x, y, 0, 1);
            }
        }
