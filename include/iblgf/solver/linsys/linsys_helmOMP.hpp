...
#ifndef IBLGF_INCLUDED_SOLVER_LINSYS_HELM_OMP_HPP
#define IBLGF_INCLUDED_SOLVER_LINSYS_HELM_OMP_HPP
...
class LinSysSolver_helm_OMP
{
  ...
  public:
    LinSysSolver_helm_OMP(simulation_type* simulation)
    ...
    void CG_solve(UcType& uc, float_type alpha)
    {
        ...
        for (int k=0; k<cg_max_itr_; k++)
        {
            ...
            if (pAp == 0.0)
            {
                float_type p_dot = dot(p,p);
                float_type ap_dot = dot(Ap, Ap);
                if (comm_.rank()==1)
                    std::cout<< "pAp too small, existing, p_mag is " << p_dot << " Ap mag is " << ap_dot << std::endl;
                return;
            }
    ...
    void CG_solve_mode(UcType& uc, float_type alpha)
    {
        ...
        for (int k=0; k<cg_max_itr_; k++)
        {
            //struct timespec tstart, t_ET_H_S_E, t_dot, t_add, t_add2;
            //clock_gettime(CLOCK_REALTIME,&tstart);
            // Ap = A(p)
            this->template ET_H_S_E<Ftmp>(p, Ap, alpha );
            //clock_gettime(CLOCK_REALTIME,&t_ET_H_S_E);
            // alpha = rsold / p'*Ap
            ...
            //clock_gettime(CLOCK_REALTIME,&t_dot);
            bool if_return = true;
            for (int i = 0; i < N_modes;i++) {
                if (pAp[i] == 0) {
            ...
            //clock_gettime(CLOCK_REALTIME,&t_add);
            add_forcing_weighted(p, r, add_ratio, 1.0);
            //clock_gettime(CLOCK_REALTIME,&t_add2);
            /*double dt_ETH = (t_ET_H_S_E.tv_sec+t_ET_H_S_E.tv_nsec/1e9)-(tstart.tv_sec+tstart.tv_nsec/1e9);
            double dt_dot = (t_dot.tv_sec+t_dot.tv_nsec/1e9) - (t_ET_H_S_E.tv_sec+t_ET_H_S_E.tv_nsec/1e9);
            double dt_add = (t_add.tv_sec+t_add.tv_nsec/1e9)-(t_dot.tv_sec+t_dot.tv_nsec/1e9);
            double dt_add2 = (t_add2.tv_sec+t_add2.tv_nsec/1e9)-(t_add.tv_sec+t_add.tv_nsec/1e9);*/

            //if (comm_.rank()==1)
            //    std::cout<< "dt_ETH = " << dt_ETH << " dt_dot = " << dt_dot << " dt_add = " << dt_add << " dt_add2 = " << dt_add2 << std::endl;
    ...
    void ET_H_S_E(VecType& fin, VecType& fout, float_type alpha)
    {
        ...
        struct timespec tstart, t_clean, t_smearing, t_helm_if, t_schur, t_add, t_project;
        ...
        clock_gettime(CLOCK_REALTIME,&t_clean);

        this->smearing<Field>(fin);

        clock_gettime(CLOCK_REALTIME,&t_smearing);
        ...
        //if (comm_.rank()==1)
        //    std::cout<< "Starting apply helm if" << std::endl;


        if (std::fabs(alpha)>1e-14)
            psolver_.template apply_helm_if_ib<Field, Field>(alpha, N_modes, this->_compute_Modes, 3, MASK_TYPE::IB2xIB);

        //clock_gettime(CLOCK_REALTIME,&t_helm_if);

        //domain_->client_communicator().barrier();

        //if (comm_.rank()==1)
        //    std::cout<< "Starting apply schur" << std::endl;


        this->template apply_Schur<Field, face_aux_type>(MASK_TYPE::xIB2IB);

        //clock_gettime(CLOCK_REALTIME,&t_schur);
        ...
        //clock_gettime(CLOCK_REALTIME,&t_project);


        /*double dt_clean = (t_clean.tv_sec+t_clean.tv_nsec/1e9)-(tstart.tv_sec+tstart.tv_nsec/1e9);
        double dt_smearing = (t_smearing.tv_sec+t_smearing.tv_nsec/1e9) - (t_clean.tv_sec+t_clean.tv_nsec/1e9);
        double dt_helm_if = (t_helm_if.tv_sec+t_helm_if.tv_nsec/1e9)-(t_smearing.tv_sec+t_smearing.tv_nsec/1e9);
        double dt_schur = (t_schur.tv_sec+t_schur.tv_nsec/1e9)-(t_helm_if.tv_sec+t_helm_if.tv_nsec/1e9);
        double dt_add = (t_add.tv_sec+t_add.tv_nsec/1e9)-(t_schur.tv_sec+t_schur.tv_nsec/1e9);
        double dt_project = (t_project.tv_sec+t_project.tv_nsec/1e9) - (t_add.tv_sec+t_add.tv_nsec/1e9);

        if (comm_.rank()==1)
            std::cout<< "dt_clean = " << dt_clean << " dt_smearing = " << dt_smearing << " dt_helm_if = " << dt_helm_if << " dt_schur = " << dt_schur
            << " dt_add =  " << dt_add << " dt_project " << dt_project << std::endl;*/
    ...
    void apply_Schur(int fmm_type)
    {
        ...
        // apply L^-1
        if (fmm_type == MASK_TYPE::xIB2IB || fmm_type == MASK_TYPE::IB2xIB) psolver_.template apply_lgf_and_helm_ib<cell_aux2_type, cell_aux2_type>(N_modes, this->_compute_Modes, 1, fmm_type);
        else psolver_.template apply_lgf_and_helm<cell_aux2_type, cell_aux2_type>(N_modes, 1, fmm_type);
