...
#ifndef IBLGF_INCLUDED_PETSC_SPECTRAL_MAT_HPP
#define IBLGF_INCLUDED_PETSC_SPECTRAL_MAT_HPP
...
struct NS_AMR_LGF : public SetupNewton<NS_AMR_LGF, parameters>
{
	...
	float_type run(int argc, char *argv[])
	{
		...
		linear_ns_t ifherk(&this->simulation_);
        ...
        if (world.rank() != 0) ifherk.Curl_access<u_ref_type, w_ref_type>();
        if (world.rank() != 0) ifherk.nonlinear_Jac_access<u_type, u_ref_type, N_ref_type>();
        if (world.rank() != 0) ifherk.AddSmearing<N_ref_type>(forcing_ref);
        if (world.rank() != 0) ifherk.div_access<N_ref_type, cs_ref_type>();
        if (world.rank() != 0) ifherk.up_and_down<cs_ref_type>();
        if (world.rank() != 0) ifherk.pad_pressure<cs_ref_type, p_ref_type>();
        if (world.rank() != 0) ifherk.Upward_interpolation<cs_ref_type, cs_ref_type>();
        ...
        if (world.rank() == 0) {
            ...
            ifherk.Grid2CSR<u_ref_type, p_ref_type, w_ref_type, N_ref_type, cs_ref_type>(loc_vec, forcing_ref, false);
        }
        ...
        ifherk.CSR2Grid<u_num_type, p_num_type, w_num_type, N_num_type, cs_num_type>(res, forcing_num);
        ...
        ifherk.Grid2CSR<u_tar_type, p_tar_type, w_tar_type, N_tar_type, cs_tar_type>(res_tar, forcing_tar);
        ifherk.Grid2CSR<u_num_type, p_num_type, w_num_type, N_num_type, cs_num_type>(res_num, forcing_num);
        ...
        ifherk.CSR2Grid<error_u_type, error_p_type, error_w_type, error_N_type, error_cs_type>(errvec, errVec);
        ...
        Vec            x, b, b1, u, b_nest, res_nest, y_nest;          /* approx solution, RHS, exact solution */
        //Vec*           Cv;               /*deflation of 
        Mat            A, B, C[N_sp*N_sp], D;             /* linear system matrix */
        ...
        for (int i = 0; i < N_sp * N_sp; i++) {
            C[i] = NULL;
        }
        for (int i = 0; i < N_sp; i++) {
            PetscCall(MatCreateAIJ(PETSC_COMM_WORLD, nlocal, nlocal, n, n, NULL, d_nnz, NULL, o_nnz, C + (N_sp + 1) * i));
            PetscCall(MatAssemblyBegin(C[(N_sp + 1) * i], MAT_FINAL_ASSEMBLY));
            PetscCall(MatAssemblyEnd(C[(N_sp + 1) * i], MAT_FINAL_ASSEMBLY));
        }
        ...
        for (int i = 0; i < N_sp; i++) {
            PetscCall(MatDuplicate(A, MAT_COPY_VALUES, (C + (N_sp + 1) * i)));
        }

        PetscCall(MatCreateNest(PETSC_COMM_WORLD, N_sp, NULL, N_sp, NULL, C, &D));
        ...
        // Create a new nested vector of 1s and apply the nested matrix to it
        MatCreateVecs(D, &b_nest, &y_nest);
        MatCreateVecs(D, &res_nest, NULL);
        VecSet(b_nest, 1.0);
        PetscCall(MatMult(D, b_nest, y_nest));

        PetscCall(KSPCreate(PETSC_COMM_WORLD, &ksp));
        PetscCall(KSPSetOperators(ksp, D, D));
        ...
        PetscCall(KSPSetTolerances(ksp, 1.e-10, PETSC_DEFAULT, PETSC_DEFAULT,
            PETSC_DEFAULT));

        PetscCall(KSPSetFromOptions(ksp));

        PetscCall(KSPSolve(ksp, y_nest, res_nest));

        PetscCall(KSPView(ksp, PETSC_VIEWER_STDOUT_WORLD));

        PetscCall(VecAXPY(res_nest, -1.0, b_nest));
        PetscCall(VecNorm(b_nest, NORM_2, &norm));
        if (rank == 1) { std::cout << "solution mag is " << norm << std::endl; }
        ...
        {
            PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                "Norm is %g, Iterations %" PetscInt_FMT "\n",
                (double)norm, its));
        }
