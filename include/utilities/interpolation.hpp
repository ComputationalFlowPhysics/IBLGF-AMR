                              ...
                              float_type x, float_type  y, float_type z,const Field& _field )
{
    ...
    */
    ...
    const float_type z_ref = z - z_pos[0];
    {
        ...
        //    for (unsigned int j=0; j<N; ++j)
        //    {
        //        auto nn = l->nodes[l->index(x_pos[i]%nx, y_pos[j]%ny)];
        //        y_values[j] = transform(nn, p, std::sqrt(nn.Temp()/velocity_set().T0), nn.u(), nn.v(), s0, u0, v0);
        //    }
        //    x_values[i] = lb::interpolate(y_ref, y_values);
        //    //x_values[i] = weno_interpolate(y_ref, y_values);
        //}
        //return lb::interpolate(x_ref, x_values);
